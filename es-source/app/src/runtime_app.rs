...
use crate::driver::HttpEsSourceDriver;
use crate::service::EsSourceService;

pub struct EsSourceRuntimeApp {
...
impl EsSourceRuntimeApp {
    pub fn new(
        ...
        Self {
            runtime_boundary,
            data_boundary,
            origin_id: NodeId("es-source".into()),
            fetch_task: Mutex::new(None),
        }
    }

    fn manifest_config(&self) -> Result<EsSourceRuntimeConfig> {
        let cfg = self
            .runtime_boundary
            .app_manifest_config()
    ...
    fn local_node_id(&self) -> String {
        ...
        let Some(capanix_app_sdk::runtime::ConfigValue::Map(runtime)) = cfg.get("__cnx_runtime")
        else {
            return self.origin_id.0.clone();
        };
        match runtime.get("local_host_ref") {
            Some(capanix_app_sdk::runtime::ConfigValue::String(value))
    ...
    fn stop_fetch_task(slot: &mut Option<RuntimeServerTask>) -> Result<()> {
        ...
        match task.join.join() {
            Ok(result) => result,
            Err(_) => Err(CnxError::Internal("es-source fetch task panicked".into())),
        }
    }
}

#[async_trait]
impl RuntimeBoundaryApp for EsSourceRuntimeApp {
    async fn start(&self) -> Result<()> {
        let config = self.manifest_config()?;
        let service = Arc::new(EsSourceService::new(
            config,
            Arc::new(HttpEsSourceDriver::default()),
        ...
        let join = thread::Builder::new()
            .name("es-source-fetch".into())
            .spawn(move || {
                let runtime = tokio::runtime::Builder::new_current_thread()
                    .enable_all()
                    .build()
                    .map_err(|err| {
                        CnxError::Internal(format!("es-source fetch runtime init failed: {err}"))
                    })?;
            ...
            .map_err(|err| {
                CnxError::Internal(format!("es-source fetch thread spawn failed: {err}"))
            })?;
        let mut guard = self
            .fetch_task
            .lock()
            .map_err(|_| CnxError::Internal("es-source fetch task lock poisoned".into()))?;
    ...
    async fn send(&self, _events: &[Event], _timeout: Duration) -> Result<()> {
        Err(CnxError::NotSupported(
            "es-source data plane is served through bound internal route".into(),
        ))
    }

    async fn recv(&self, _opts: RecvOpts) -> Result<Vec<Event>> {
        Err(CnxError::NotSupported(
            "es-source does not expose a stream recv surface in v1".into(),
        ))
    }
