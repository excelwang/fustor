...
async def test_user_crud_flow(authorized_client):
    # 1. 获取用户
    response = authorized_client.get(base_path)
    ...
    response = authorized_client.put(base_path, json=update_payload)
    assert response.status_code == 200, response.text

async def test_user_change_password(authorized_client):
    # Test successful password change
    valid_payload = {
        "old_password": "pwdtesT1!",
        "password": "pwdtesT2!"
    }
    response = authorized_client.put(f"{base_path}/password", json=valid_payload)
    assert response.status_code == 200, response.text
    ...
    response = authorized_client.put(f"{base_path}/password", json=invalid_payload)
