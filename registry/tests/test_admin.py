...
async def test_user_crud_flow(admin_test_client):
    # 1. 创建用户（需要管理员权限）
    payload = {
        "name": "newuser",
    ...
    response = await admin_test_client.post(f"{base_path}/users", json=payload)
    assert response.status_code == 201, response.json()
    
    user_id = response.json()["id"]
    
    # 2. 获取用户
    response = await admin_test_client.get(f"{base_path}/users/{user_id}")
