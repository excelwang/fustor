...
- For cluster convergence, use a single 300s local observation window inside the launched command before one bounded remote sample, then use one 300s `write_stdin` continuation to wait for that window result. Example shape: `sleep 300; ssh ...one bounded sample...`. The `sleep` must appear before `ssh`; do not put long `sleep` inside an SSH heredoc or remote script.
- Do not launch fixed 900s local sleeps for background convergence waits. They force three 300s execution-session continuations with no possible sample output. If more patience is required, run another explicit 300s observation window only after the previous sample has been classified and gives a concrete reason to keep waiting.
- Forbidden wait shapes: `sleep 900; ssh ...`, `ssh host 'sleep 900; ...'`, `ssh host 'bash -s' <<'REMOTE'` with a long remote `sleep`, `while sleep 30; do ssh ...; done`, repeated local `date`, repeated `ps`/`pgrep`, repeated log tails, repeated SSH status samples, status-only replies, or ordinary goal continuations that only restate the wait contract.
