...
async def test_echo_driver_push(caplog):
    ...
    events = [InsertEvent(schema="test_schema", table="test_table", rows=[{"id": 1, "msg": "hello"}])]
...
async def test_echo_driver_cumulative_push(caplog):
    ...
    # First batch
    events1 = [
        InsertEvent(schema="test_schema", table="files", rows=[{"id": 1}, {"id": 2}]),
    ]
    
    # Second batch
    events2 = [
        InsertEvent(schema="test_schema", table="files", rows=[{"id": 3}])
    ]
