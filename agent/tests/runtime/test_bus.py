...
class TestMemoryEventBus:
    async def test_put_and_get_events(self):
        ...
        event1 = InsertEvent(schema="s", table="t", rows=[{'id': 1}], index=0)
        event2 = InsertEvent(schema="s", table="t", rows=[{'id': 2}], index=1)
    ...
    async def test_buffer_capacity(self):
        ...
        await bus.put(InsertEvent(schema="s", table="t", rows=[{'id': 1}], index=0))
        await bus.put(InsertEvent(schema="s", table="t", rows=[{'id': 2}], index=1))

        put_task = asyncio.create_task(bus.put(InsertEvent(schema="s", table="t", rows=[{'id': 3}], index=2)))
    ...
    async def test_trim_buffer(self):
        ...
        for i in range(5):
            await bus.put(InsertEvent(schema="s", table="t", rows=[{'id': i}], index=i))
    ...
    async def test_can_subscribe(self):
        ...
        await bus.put(InsertEvent(schema="s", table="t", rows=[{'id': 5}], index=5))
        await bus.put(InsertEvent(schema="s", table="t", rows=[{'id': 6}], index=6))
