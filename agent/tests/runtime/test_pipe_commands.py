...
from fustor_agent.runtime.pipe.command import PipeCommandMixin

class MockPipe(PipeCommandMixin):
    def __init__(self):
        self.id = "test-pipe"
...
async def test_handle_command_scan():
    ...
    await pipe._handle_commands([cmd])
...
async def test_handle_command_update_config(tmp_path):
    ...
    with patch("fustor_core.common.get_fustor_home_dir", return_value=tmp_path):
        with patch("os.kill") as mock_kill:
            ...
            # Now validate_config exists
            pipe._handle_command_update_config(cmd)
...
async def test_handle_command_report_config(tmp_path):
    ...
    with patch("fustor_core.common.get_fustor_home_dir", return_value=tmp_path):
        pipe._handle_command_report_config({"filename": "default.yaml"})
        await asyncio.sleep(0.1)
