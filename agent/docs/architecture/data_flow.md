...
1.  **消息优先 (Message-First)**: 系统的默认运行模式是**消息阶段 (Message Phase)**。Pipeline 任务一旦启动，会立即开始监听和推送实时数据流，确保新数据的同步延迟达到最小化。
...
4.  **自主启动并发快照**: `AgentPipeline` 在收到 `needed_position_lost=True` 信号后，得知源端发生了数据丢失。它会立即使用 `asyncio.create_task()` **异步启动**一个 `_run_snapshot_sync` 任务，该任务负责在后台独立地进行历史数据回填。
