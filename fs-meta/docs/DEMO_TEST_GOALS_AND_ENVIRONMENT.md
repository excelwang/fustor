...
diff --git a/fs-meta/docs/DEMO_TEST_GOALS_AND_ENVIRONMENT.md b/fs-meta/docs/DEMO_TEST_GOALS_AND_ENVIRONMENT.md
new file mode 100644
index 0000000..42927a6
--- /dev/null
+++ b/fs-meta/docs/DEMO_TEST_GOALS_AND_ENVIRONMENT.md
@@ -0,0 +1,179 @@
...
+### 3.4 当前 NFS 数据源
+
+当前业务数据源来源于 5 个 NFS 目录关系。具体 host 和 mount point 不提交到仓库，由本地环境变量注入：
+
+- `FSMETA_DEMO_HOST_1` + `FSMETA_DEMO_MOUNT_1`
+- `FSMETA_DEMO_HOST_2` + `FSMETA_DEMO_MOUNT_2`
+- `FSMETA_DEMO_HOST_3` + `FSMETA_DEMO_MOUNT_3`
+- `FSMETA_DEMO_HOST_4` + `FSMETA_DEMO_MOUNT_4`
+- `FSMETA_DEMO_HOST_5` + `FSMETA_DEMO_MOUNT_5`
+
+这些值应写入本地未提交的 env 文件，并用 `fs-meta/docs/examples/render-monitoring-roots-5group.sh` 生成 roots JSON。
+
+### 3.5 当前 facade 入口
+
+当前 `fs-meta` facade 入口由本地环境变量指定：
+
+- `FSMETA_DEMO_API_BASE`
+
