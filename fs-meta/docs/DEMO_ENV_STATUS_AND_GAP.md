...
已在 `10.0.82.144~148` 上准备并运行兼容当前 CentOS 7 / glibc 环境的 `capanix` 二进制。
...
`10.0.82.145:18080` 上的 `fs-meta` API 已经可以对外提供标准 HTTP 服务。
...
虽然 5 节点底座正常，且多个节点本地 `resource announce` 已成功，但当前 `10.0.82.145` 上运行中的 `fs-meta` 所看到的 `/runtime/grants` 仍然只有本机资源：
...
- 当前 active facade 所在节点是 `10.0.82.145`
