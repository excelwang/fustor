...
| `crates/host-adapter-fs-meta/specs` | fs-meta-specific bound-host facade ownership | host-fs facade、watch/session、post-bind dispatch wording | fs-meta 通过 `HostFsFacade` 消费本地 facade；不把 dispatch tags、watch descriptors、session ids 提升为 runtime/platform capability vocabulary | aligned |
| `crates/host-fs-types/specs` | shared DTO/query vocabulary ownership | query vocabulary、typed event continuity | fs-meta 继续拥有产品 HTTP envelope，而 shared DTO 和 query vocabulary 保持在 `host-fs-types` | aligned |
