...
18. source publication backpressure is lane-specific: scan/audit output MUST stay bounded, and downstream sink/channel pressure MUST propagate back to scan/audit read-ahead plus source scan/audit concurrency/admission state rather than being absorbed by an unbounded source-side publication buffer.
19. realtime watch ingestion MUST NOT be blocked behind scan/audit backlog or scan/audit publication backpressure. Realtime watch events use a separate publication lane that can bypass queued scan/audit batches.
20. source MUST NOT mark scan/audit publication progress as downstream-accepted, or release the scan/audit concurrency that should be throttled by that batch, while the batch is only parked in an unbounded or fairness-only source-side buffer.
21. if the realtime lane cannot enqueue or deliver a watch batch, source MUST surface explicit overflow/backpressure evidence such as `ControlEvent::WatchOverflow`, `overflow_pending_materialization`, or root/group degraded reason `HOST_FS_BACKPRESSURE`, and MUST rely on the existing audit/repair path for convergence. It MUST NOT silently drop realtime events while preserving trusted observation readiness, and MUST NOT block the host watch loop on sink progress as the recovery mechanism.
...
6. sink/channel backpressure on the scan/audit publication lane is valid flow control and MUST throttle source scan/audit read-ahead plus source concurrency/admission state instead of permitting unbounded source memory growth or progress accounting ahead of downstream acceptance.
7. scan/audit backpressure MUST NOT be forwarded to realtime host watch ingestion. Realtime watch has its own bounded nonblocking publication lane; when that lane is full, the system records explicit backpressure/overflow evidence and converges through audit/repair rather than blocking the host watch loop on sink progress.
...
4. “已分配物化负责人”只表示运行放置已就绪；只有对应 group 已产出同 epoch 的 sink 物化证据并达到 ready，才可以把 trusted observation readiness 判定为 ready。sink-status 必须先报告该 sink runtime 已接受的 active group schedule；不能在 status 层再用可能滞后的 host-grant 投影把已分配给本 runtime 的 group 隐藏掉。
...
14. node-scoped manual-rescan delivery treats a zero-evidence route exchange as route-convergence pending while the bounded delivery window remains open. The API reissues only the same idempotent target-scoped request and still requires explicit `accepted` evidence from the named source owner before accepting the operation.
15. manual-rescan broad runtime control stream records source-primary owners' manual-rescan intent but does not wake scan/audit traversal before scoped/generic request-reply delivery acceptance is proven. The accepted request-reply path owns the first scan wake so file count, NFS behavior, or audit duration cannot affect target acceptance.
16. while runtime control is already fail-closed, cleanup-only query and per-peer sink logical-roots deactivates are absorbed into retained route state locally; they do not re-enter the sink worker bridge and do not keep management writes unavailable behind retired worker RPCs. Cleanup-only sink events stream deactivate is different: it belongs to the sink worker data-ingress route and may be sent to the sink worker by itself, but it must not carry retained sink activate replay or drag app-side query/logical-roots cleanup into the worker lane.
17. when source/sink worker-control recovery exhausts its bounded retry budget, the app keeps the process boundary alive, marks runtime control uninitialized, retains replay state, and reports degraded/unavailable evidence through app status/query gates instead of returning a process-level `Timeout`, `ChannelClosed`, or `TransportClosed` from `on_control_frame`.
18. blocking source-repair/source-observability access paths and management-write readiness waits are recovery entrypoints: if retained source/sink replay is pending and no control op is already in flight, that blocking access path performs one bounded retained-state replay repair before returning `not started` or management-write `not ready`; source/sink recovery does not rely on a later runtime tick after a generation cutover. Generic nonblocking source-status must first return current runtime-scope control-cache evidence when it already has source/scan scheduled groups and control-route facts for the same authority epoch; that status response is ownership evidence, not replay completion, and it must not consume retained replay or classify control-inflight ownership evidence as worker-unavailable before publishing the current runtime scope. A pending post-rescan publication refresh must withhold live root-health, source-primary, and publication counters from that cache response, but it must not erase source/scan ownership maps needed by release-upgrade and status fan-in gates. After retained replay is current and a live worker client exists, runtime-scope control cache is no longer final data truth; normal source-status takes a bounded live observation inside the caller's status-route collection attempt budget. If that bounded live observation fails or times out while current runtime-scope control-cache evidence still exists, generic source-status returns the runtime-scope ownership evidence with explicit cache provenance instead of erasing ownership as worker-unavailable. Stable root-health cache may skip the live read only after publication evidence exists for the cached runtime scope. Public status readiness is per-root: `trusted_observation_readiness` opens when current logical roots have trusted source evidence and ready sink materialization, even if a merged peer response still carries runtime-scope control-cache provenance. Cache-only worker status, pending source state, and real degraded roots remain blocking. Management `/status` sink fan-in is nonblocking facade evidence, not a worker lifecycle repair loop: it may use one existing-client sink probe inside the short status-route budget, but it must return cached/degraded sink evidence on timeout, reset, or missing-client evidence instead of starting/retrying the worker behind a fixed-bind handoff. A status-triggered source-scoped sink observation repair MAY be scheduled only as one nonblocking retained-state replay wake when current source-owned runtime-scope evidence proves an accepted sink owner route is missing local sink observation; its duplicate suppression key MUST include the accepted sink-control signal shape plus the app-authoritative logical-roots declaration content and generation. Repeated `/status` polls with the same accepted repair signature MUST return observation evidence without replaying the same sink control again; a newer logical-roots declaration generation or content may reopen exactly that retained replay. This duplicate suppression is scoped to the status-observation repair lane. The owning full sink-generation-cutover/background recovery lane has its own bounded retry deadline, and while retained sink replay remains open with missing owner-scoped sink evidence, it MAY republish the same logical-roots declaration to the current owner-scoped sink roots-control routes. This repair MUST NOT fabricate sink ownership from local grants or schedule-only evidence, MUST NOT enter worker start/retry paths, and MUST NOT be awaited by the `/status` response.
   For management `/status`, the recovery-entrypoint rule is narrower than write-readiness and explicit runtime-readiness probes. After a source/sink route timeout, transport collapse, missing-client evidence, or retained sink-generation-cutover ownership, `/status` records degraded, partial, or not-ready evidence and returns within its response budget. It MUST NOT synchronously run status-only retained replay, worker start/retry, or full sink repair for those cases. Status-triggered sink observation repair is allowed only after sink route collection itself succeeds (`sink_route=ok`) and current source-owned runtime-scope evidence proves accepted sink owner observation is incomplete; if `sink-generation-cutover` or another full sink replay lane already owns that accepted signal signature, `/status` may at most issue a nonblocking wake and then observe the resulting evidence.
19. source-to-sink convergence pretrigger runs only for the first mixed source/sink boot wave with no retained replay pending; recovery waves do not submit that synchronous pretrigger before retained replay has reopened the control gate.
20. after source control fail-closes inside a mixed source/sink generation-cutover frame, the app returns from the current control frame with retained source/sink replay armed; it must not continue into same-frame sink replay or sink cleanup RPCs behind the failed source lane.
21. after that fail-closed cutover, later tick, cleanup, or empty process wakeups that still carry retained post-initial source or sink replay must keep the latest desired state and return without inline worker replay RPCs; retained replay includes active routes and ordinary deactivating route state. A restart-deferred retire cleanup that removes the last active source route is absorbed at the app endpoint gate and clears source worker replay, because no source worker route remains to repair. A later non-empty source recovery wave may run one bounded source apply; retained sink replay is driven by bounded recovery entrypoints instead of process apply/quorum wakeups, with management `/status` limited to the narrow observation-repair predicate and nonblocking wakes described here.
22. if that fail-closed app owns a fixed-bind HTTP facade, it records the failed-owner state with the active bind owner; when a successor later registers a pending claim for the same bind address, the successor releases that stale owner through the bounded fixed-bind handoff path instead of leaving a not-ready facade bound forever.
23. if the fail-closed app is the pending successor and the predecessor is already only continuity-retaining a runtime-deactivated fixed listener, the successor releases the retained predecessor through the same bounded handoff path and exposes its own fail-closed app boundary, so new API requests do not continue through a drained/fenced predecessor PID. While the successor has a pending fixed-bind facade that is not runtime-exposure-confirmed, older cleanup-only facade deactivates must retain the currently active listener instead of shutting down the only reachable API facade. During this cutover window, service-unavailable manual-rescan current-roots readiness failures are retryable convergence evidence for multi-facade operator clients inside their bounded management retry window; retry/failover may only move the request to another candidate facade and MUST NOT accept repair without fresh source-owned readiness and scoped delivery proof.
24. after a successor fixed-bind HTTP facade is actively serving, cleanup-only query/facade tails from the failed cutover may withdraw stale auxiliary source-status/source-find lanes, but they must not withdraw the active facade's core business read lanes (`find`, materialized query proxy) while runtime control is still fail-closed; those lanes belong to the serving facade and must survive until retained replay either recovers them or a later initialized business route change supersedes them. The same continuity rule applies inside the app boundary when source/sink retained replay is deferred by generation cutover: an already-active materialized query proxy route remains receive-capable while the current source/sink desired state is replayed below it, rather than closing the reusable request/reply route and stranding the next selected-group read. `sink-status` is sink-owner readiness evidence and follows active sink runtime owners rather than facade ownership. Trusted-materialized fan-in must derive current sink-owner candidates from the same routed source-observability collection that established the active read roots before falling back to local cache. A current sink runtime owner must serve the generic sink-status lane and its owner-scoped sink-status lane after activation; the app must not rely on route-unit declaration alone to prove trusted observation fan-in coverage.
25. steady-state control, status, and endpoint request diagnostics are opt-in or bounded summaries by default; full signal payloads and per-batch normal-path logs are debug evidence, not part of the operations workload being measured by CPU gates.
26. while retained source or sink replay is pending, tick/cleanup wakeups must not reopen normal runtime initialization before the retained replay recovery lane clears the app gate; otherwise a fail-closed owner can rebuild endpoints on every steady tick and turn recovery bookkeeping into steady CPU workload.
27. once runtime control is already initialized, steady tick/keepalive waves must not re-enter the runtime initialization path; initialization is reserved for first activation or bounded recovery, not normal keepalive processing.
28. after a post-reset recovery, current retained-generation source ticks may drive one bounded retained-state repair and must keep the tick evidence; this is distinct from steady keepalive processing and from older retained generation-cutover wakeups.
29. if runtime control remains fail-closed only because sink replay is still pending, source current-generation keepalive ticks are retained gate evidence only; they must not re-enter source-worker control while source replay is already clear.
30. status-route fan-in may reach the same source worker through multiple active route units; same-node nonblocking source observability reads that overlap in one status cycle share one live worker read and use the resulting fresh cache for followers, instead of multiplying source-worker RPCs per status request. A nonblocking source-status read that can answer from current runtime-scope control cache must not block behind retained replay or an already-running control turn. If a post-rescan live publication refresh or first live publication proof is still required, the cache response may report only runtime-scope ownership and route-control provenance; it must not report that pending publication as live root-health or materialized data readiness. Manual-rescan pre-delivery probes and management-write readiness remain the blocking recovery paths.
   Deployment-scoped and owner-scoped source-status/sink-status fan-in inside `/status` are short-budget observation steps. They may improve a partial local or generic reply, but slow peers degrade the current response instead of accumulating multiple serving-state status-route waits. Manual-rescan pre-delivery probes and management-write readiness may keep their stronger budgets because they are explicit write/repair entrypoints, not ordinary status polling.

31. when a retained source or sink replay is deliberately deferred at the app boundary, the app marks only that lane as still needing replay; deferring sink replay must not re-arm already-cleared source replay, and deferring source replay must not re-arm already-cleared sink replay.
32. management write-readiness and explicit runtime-readiness probes are the strong bounded recovery entrypoints: if retained source or sink replay is pending, the app must run one bounded replay repair before returning `NotReady` and clear the app-level replay flag once the worker no longer needs retained replay; empty control frames, cleanup frames, ordinary tick frames, and ordinary management `/status` polls remain process-apply or observation evidence and do not own this repair. Runtime-unit exposure confirmation is an explicit runtime-readiness probe: after source replay is clear, it may run the retained sink replay repair once instead of deferring it again as a process-apply wakeup. If a concurrent recovery/control turn has already made the worker retained-replay state current, the management-write recovery clears the app gate from that worker replay flag before issuing blocking status snapshots, because a stale status/read bridge is not allowed to keep management writes unavailable after replay ownership has already converged.
   Management `/status` is a weak observation entrypoint: it may clear already-current replay flags or issue a nonblocking singleflight wake for the narrow status-triggered observation repair after status route collection has already succeeded and the predicate above is true. For source/sink route timeout, transport collapse, missing-client evidence, or a replay signature already owned by sink-generation-cutover/full sink replay, `/status` returns degraded, partial, or not-ready evidence inside its request budget instead of running blocking retained replay.
   Every repair/replay lane touched by `/status` must leave structured repair-lane evidence on the response path, including fail-closed responses: source repair, management-write recovery, status-triggered sink observation repair, sink-generation-cutover/full replay ownership, and scan/audit admission release or refusal must be distinguishable without log scraping. Background source/sink recovery lanes scheduled after generation cutover must also publish durable lane snapshots with scheduled/inflight/completed/failed/timed-out state so operators can observe them through `/status` without reading process logs. Each lane snapshot must carry a stable diagnostic signature and update timestamp; generation, attempt, and deadline must be populated when the lane owns a meaningful authority generation, retry attempt, or bounded recovery deadline. Scan/audit admission release is a nonblocking status wake: `/status` reports scheduled/inflight/completed/blocked evidence and returns without waiting for owner-routed release calls to complete. The evidence is diagnostic state, not an authorization source for ownership or trusted readiness.
33. recovery progress waits use source/sink snapshots as evidence probes inside a bounded observation window; a probe that returns quickly without a runtime-change notification is not progress and must not turn retained replay recovery into a tight poll loop. After retained sink replay restores current scope, scheduled sink status rows with pending materialization or mixed ready/unready evidence are catch-up evidence, not a terminal control-frame failure; the app defers gate reopening/republication completion and lets the materialization lane continue.
34. when an authoritative retained sink control wave successfully covers the current retained sink desired state after a worker reset, the sink worker bridge marks that retained replay current. Later steady ticks then stay on the keepalive fast path instead of replaying the full retained sink state on every tick/status cycle.
35. sink keepalive ticks may expand into retained sink route state only when app-level or worker-level retained sink replay is explicitly required; a normal current-generation tick is liveness evidence, not authority to replay every retained sink route.

## [workflow] AsyncRecoveryLaneObservation

**Steps**

1. source/sink/facade recovery owners publish lane state whenever retained replay, roots-control propagation, worker-control recovery, source repair, manual-rescan execution, scan/audit traversal, or sink materialization catch-up is scheduled, starts, blocks, completes, fails, times out, or is skipped.
2. each lane update carries the lane name, owner plane, `RecoveryLaneState`, trigger, diagnostic signature, update timestamp, and any meaningful authority generation, retry attempt, deadline, route outcome, or reason.
3. `/status` reads the latest lane snapshots from the app-local recovery lane registry and merges them with lane evidence created during the current request's route collection.
4. if `/status` observes a missing prerequisite, timeout, missing client, or already-owned retained replay, it reports degraded/not-ready status plus lane evidence and returns within the status request budget.
5. `/status` may issue a nonblocking wake to an existing lane when doing so does not make the response wait for repair completion. The wake must first claim the lane signature in the recovery lane registry; if that signature is already `scheduled` or `inflight`, `/status` returns the existing lane evidence and does not spawn duplicate recovery.
6. `/status` may schedule only the narrow status-observation repair predicate as a nonblocking single-flight wake: sink route collection succeeded, source-owned runtime-scope evidence proves accepted sink ownership, local sink observation is missing, and no full sink replay lane already owns that accepted signal signature.
7. source scan/audit, manual-rescan execution, worker start/retry, retained source/sink replay, roots-control full replay, and sink materialization catch-up are executed by their owning recovery lanes, not by the status request path.
8. clients and tests treat `repair_lanes[]` as the supported observation interface for asynchronous recovery progress; absence of logs is not absence of recovery, and logs are not a substitute for lane evidence.
...
1. trusted-materialized `/tree` and `/stats` first check whether root-level sink schedule or cached sink status evidence exists for the current authority epoch. In a routed facade, local sink schedule absence is not authoritative because the current materialized owner may be a peer sink; the routed path must perform peer sink-status fan-in before deciding that no current schedule exists. For unscoped root reads, the expected readiness groups come from the app-authoritative current scan-enabled logical roots; a partial routed source-status fan-in response is convergence evidence only and must not shrink that root-readiness group set while the response still returns all groups.
...
5. full demo environment baseline fails when artifact mismatch, host-fs timeout/backpressure, missing metadata coverage, wrong root/group coverage, or source degradation remains. Sink materialization unreadiness is recorded as bounded catch-up evidence at this L3 environment baseline, because full real-NFS data volume is intentionally large and unpredictable; trusted-materialized `/tree` and `/stats` remain fail-closed until sink readiness covers the active roots.
6. strict materialized/operations acceptance fails when materialization unreadiness remains; this stricter gate belongs to small deterministic materialized checks or L5 operations gates, not to the L3 full real-NFS environment baseline.
7. full real-NFS operations use a runtime channel budget sized for the full five-node route set plus one release-upgrade generation overlap; mini/debug channel caps must not be reused for full operations gates.
8. release-upgrade operations gates that trigger manual rescan or steady CPU sampling must wait for app-visible source runtime-scope evidence or accepted manual-rescan delivery evidence for the current roots; source/scan process liveness alone is not convergence evidence. Source runtime-scope evidence proves coverage by the app-selected owner nodes for the current logical roots, not by a fixed demo node list, because source-primary selection is app-owned and may move with grants, topology, and generation cutover state. Source runtime-scope evidence is aggregate positive local ownership across partial peer route activations: a peer-only activation may update route state, but it must not erase local source/scan schedule evidence learned from another current wave. Empty live schedule observations are withheld from app-visible ownership; stale schedules are cleared only by explicit current-authority empty refresh for the known node. Cross-node generation comparison is stale-owner evidence only when the control scope names a concrete owner target for the group; root-id route scopes are per-node liveness facts and are not a global ordering across source owners.
9. resource-budget CPU gates measure the incremental cost of steady API polling by comparing idle and polling samples over the same live runtime process set after functional convergence. They must not compare daemon-only CPU to daemon-plus-worker CPU or fail on file-count-dependent source audit/materialization catch-up. The actionable failure is sustained polling/control churn, route respawn churn, or diagnostic work after convergence.
10. full real-NFS operations gates must preserve capanix's bounded relation-target quorum budget for multi-node startup and release cutover; fs-meta demo harnesses must not shorten that budget for the full five-node operations environment. A quorum timeout remains a fail-closed product signal, but a test-local budget below the runtime contract is not valid full-demo acceptance evidence.
...
9. after the app-authoritative roots write is accepted in a runtime-managed deployment, an operator/client followup MAY submit a Capanix scope-only relation-target update so accepted `__cnx_runtime.app_scopes` converges to the accepted roots plus the facade scope. This followup preserves the current static manifest config, target generation, worker runtime bindings, startup path, and route plans; it MUST NOT write the accepted roots into static `config.roots`, rebuild a full roots-bearing deploy declaration, or use process restart as the online reconfiguration mechanism.
10. source scan/audit admission for the new scan-enabled root set stays closed until sink accepted-scope schedule evidence covers the same groups; bounded roots followup repair or a permitted status-triggered observation repair may schedule a nonblocking release lane after sink scope convergence, but `/status` route timeout, transport collapse, missing-client evidence, or an already-owned sink-generation-cutover/full replay signature must keep the gate closed and return degraded/partial/not-ready evidence. Trusted observation readiness still requires current source publication plus sink transport/materialization evidence. Roots-apply source roots-control payloads on generic and owner-scoped source routes MUST carry the same `defer_scan_audit` admission-closed intent to peer and worker-backed sources. Manual-rescan current-roots resends MUST NOT set this roots-apply defer intent because they repair delivery proof for an already accepted declaration rather than starting a new sink-scope cutover gate.
11. realtime host watch/listen ingestion is not gated by this scan/audit admission wait. If realtime publication is full or unavailable during roots cutover, fs-meta records explicit overflow/backpressure/degraded evidence and relies on later audit repair for convergence instead of blocking the host watch loop on sink progress.
12. runtime remains the owner of bind/run realization for affected execution scopes; api/app consume the resulting runtime state and do not become bind/run semantic owners.
13. if source/sink refresh against current grants or bound scopes fails, api returns explicit internal error and reverts to the previous authoritative root set.
14. empty roots remain a valid deployed-but-unconfigured state.
...
9. api returns accepted response after cluster route delivery and scoped/generic request endpoint acceptance have submitted the needed source-primary rescan intents.
...
13. the group enters Selected-Runner Fresh Execution once the selected route yields selected-runner execution evidence: the group result is decided by that selected runner's success, explicit failure, or execution timeout. A retryable request/reply delivery gap before runner execution evidence may re-plan within the same requested group and the original caller budget; fs-meta does not keep that group open to wait for unrelated groups.
...
18. a same-group runner retry is still one group-level fresh execution decision; it must not use materialized data, unrelated group replies, or a generic fallback result that omits the requested group.
