...
6. The public availability ordering is contractual: `/on-demand-force-find` availability ⊇ materialized `/tree|/stats` availability ⊇ trusted-materialized `/tree|/stats` availability.
7. Status, degraded evidence, or diagnostics MAY explain why a narrower window is currently closed, but they MUST NOT collapse the wider `/on-demand-force-find` window into the trusted-materialized gate unless the facade itself or fresh source execution is unavailable.
