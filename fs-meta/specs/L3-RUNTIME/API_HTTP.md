...
1. `GET /status`
   ...
   7. management `/status` MAY skip optional remote source/sink fan-in only when local source evidence is live, non-degraded, and sink-ready for the same active source groups. A cached/degraded local source fallback is partial evidence and MUST keep remote fan-in open instead of publishing a smaller root or sink group set as complete status. If the deployment-scoped source-status reply omits app-authoritative active roots, `/status` MUST query the current source-owner scoped source-status routes derived from the current roots/grants before reporting the source root set; a partial generic reply is not complete cluster status. If the deployment-scoped sink-status reply omits app-authoritative active root groups, `/status` MUST query the current owner-scoped sink-status routes derived from the same current root/owner evidence before reporting the sink group set. When the preferred management/runtime boundary contributes only partial generic evidence, the query/runtime boundary remains a valid second chance for missing owner-scoped status evidence; generic success on one boundary MUST NOT suppress owner-scoped completeness checks on the other boundary.
