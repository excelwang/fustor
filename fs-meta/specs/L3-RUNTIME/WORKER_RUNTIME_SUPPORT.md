...
2. the platform generic `capanix-worker-host` loads the shared `fs-meta/app` worker module and passes `worker_role`, route key, and socket paths through the module launch payload.
