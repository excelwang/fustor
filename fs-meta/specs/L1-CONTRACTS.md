...
7. **MANUAL_RESCAN_OPERATION**: **fs-meta System** MUST provide explicit manual rescan operation to repair index drift and watch-loss conditions.
   ...
   > Verification: `pending-materialization` is not by itself an index/source drift failure. When source repair readiness and scoped source delivery readiness are already proven and the current sink status completely covers all current scan-enabled roots as only `PendingMaterialization` groups with no live/materialized nodes, manual rescan MUST fail closed before any generic or scoped rescan delivery is published. This protects high-cardinality root deployments from using manual rescan as a wait accelerator; incomplete, missing, mixed, or ready sink evidence remains outside this pending-only misuse gate and is handled by the existing readiness/fan-in gates.
