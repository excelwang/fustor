...
fn main() {
    ...
    let socket_path = match parse_socket_path(&args[1..]) {
        Ok(path) => path,
        Err(err) => {
            eprintln!("fs_meta_source_worker arg error: {err}");
            std::process::exit(2);
        }
    };
    if let Err(err) = capanix_app_fs_meta_worker_source::run_source_worker_server(&socket_path) {
        eprintln!("fs_meta_source_worker failed: {err}");
        std::process::exit(1);
    }
}
