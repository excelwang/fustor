...
fn source_control_signals_can_use_primed_schedule_timeout_fallback(
    ...
    for signal in signals {
        match signal {
            ...
            | SourceControlSignal::ManualRescan { .. }
            | SourceControlSignal::Passthrough(_) => {}
            SourceControlSignal::Deactivate { .. } => return false,
        }
    }
    saw_activate
}
...
fn primed_schedule_cache_covers_signals(
    ...
    let has_source_groups =
        cached_local_scheduled_groups(node_id, &cache.scheduled_source_groups_by_node).is_some();
    let has_scan_groups =
        cached_local_scheduled_groups(node_id, &cache.scheduled_scan_groups_by_node).is_some();
    let needs_source_groups = signals.iter().any(|signal| {
        matches!(
            signal,
            SourceControlSignal::Activate {
...
impl SourceWorkerClientHandle {
    ...
    async fn replay_retained_control_state_if_needed_for_refresh(&self) -> Result<()> {
        if self
    ...
    async fn refresh_cached_scheduled_groups_from_live_worker(&self) -> Result<()> {
        ...
        loop {
            let worker = self.worker_client().await;
            match worker.ensure_started().await {
                Ok(()) => {}
                Err(err)
                    if can_retry_on_control_frame(&err) && std::time::Instant::now() < deadline =>
                {
            ...
            self.replay_retained_control_state_if_needed_for_refresh()
                .await?;
            let worker = self.worker_client().await;
            ...
            match rpc_result {
                Ok(SourceWorkerResponse::Ack) => {
                    ...
                    if should_refresh_cached_schedule {
                        ...
                        if let Err(err) = self
                            .refresh_cached_scheduled_groups_from_live_worker()
                            .await
                        {
                            let can_fail_closed_from_primed_cache = matches!(
                                ...
                                .is_some_and(|signals| {
                                    !replay_required_before_refresh
                                        ...
                                        primed_schedule_cache_covers_signals(
                                            cache,
                                            &self.node_id,
                                            signals,
                                        )
                            ...
                            if can_fail_closed_from_primed_cache {
                                eprintln!(
...
mod tests {
            ...
            let replace = match best.as_ref() {
                ...
                Some((best_modified, best_index, _)) => {
                    modified > *best_modified
                        || (modified == *best_modified && index < *best_index)
                }
            };
            if replace {
    ...
    async fn start_recreates_shared_worker_client_when_retryable_start_resets_never_converge() {
        ...
        tokio::time::timeout(Duration::from_secs(8), client.start())
            .await
            .expect(
                "source worker start should swap off a permanently resetting shared worker client",
            )
    ...
    async fn single_restart_deferred_retire_pending_source_roots_deactivate_fails_fast_after_repeated_bridge_reset_errors()
     {
        ...
        let cleanup_envelopes = vec![
            encode_runtime_exec_control(
                &RuntimeExecControl::Deactivate(capanix_runtime_entry_sdk::control::RuntimeExecDeactivate {
                    route_key: format!("{}.stream", ROUTE_KEY_SOURCE_ROOTS_CONTROL),
                    unit_id: SOURCE_RUNTIME_UNIT_ID.to_string(),
                    lease: None,
                    generation: 3,
    ...
    async fn tick_only_followup_replays_retained_fs_source_selected_scopes_after_generation_two_reconnect()
     {
        ...
        .expect("raw observability snapshot after tick-only reconnect")
        {
            ...
            other => panic!(
                "unexpected source worker response for raw observability snapshot: {other:?}"
            ),
        };
    ...
    async fn generation_two_real_source_route_post_ack_schedule_refresh_aborts_inflight_stale_prerebind_logical_roots_snapshot()
     {
        ...
        client
            .on_control_frame(source_wave(2))
            .await
            .expect("generation-two real source route replay should recover after shared-client rebind");
