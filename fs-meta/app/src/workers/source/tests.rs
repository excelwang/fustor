...
fn source_control_frame_machine_owns_generation_one_timeout_reset_retry_lane() {
    ...
    let action = machine.action_after_error(
        CnxError::Timeout,
    ...
    let terminal_reconnect = machine.action_after_error(
        ...
        matches!(
            machine
                .advance(SourceControlFrameEvent::RpcCompleted {
                    ...
                    before: SourceProgressState::default(),
...
fn source_control_frame_test_machine() -> SourceControlFrameMachine {
    SourceControlFrameMachine {
        ...
        replay_required_for_current_wave: false,
...
#[test]
fn source_replay_retained_control_state_machine_reconnects_then_retries_replay() {
    let mut machine = SourceReplayRetainedControlStateMachine::new(
        source_machine_test_future_deadline(),
        SOURCE_WORKER_SCHEDULE_REFRESH_RPC_TIMEOUT,
    );
    let after_gap = SourceProgressState {
        worker_client_replaced_epoch: 7,
        ..SourceProgressState::default()
    };
    ...
    assert!(
        ...
        "source retained-control replay recovery must reconnect, wait for the replacement worker, and retry retained route-state replay inside the bounded repair lane",
    );
}
...
fn source_retained_replay_control_rpc_uses_single_started_attempt() {
    ...
    assert!(
        executor.contains("SourceControlFrameRequestAttemptKind::ControlFrame => {")
            ...
            && executor.contains("self.with_started_once_with_failure(|client|"),
        "source control-frame attempts must use one started worker RPC attempt and leave retry/reconnect policy to the source control-frame state machines",
    );
}
...
#[test]
fn targeted_rescan_acceptance_is_bound_to_current_worker_client() {
    let source_impl = include_str!("../source.rs");
    assert!(
        source_impl.contains("async fn ensure_source_worker_client_still_current(")
            && source_impl.contains(".client_is_current(client)"),
        "source worker client must use capanix current-client evidence for scoped rescan acceptance"
    );
    ...
    for func in [
        ...
        let start = source_impl
            .find(func)
            .unwrap_or_else(|| panic!("missing source worker client function: {func}"));
        let tail = &source_impl[start + func.len()..];
        let end = tail.find("\n    async fn ").unwrap_or(tail.len());
        let body = &source_impl[start..start + func.len() + end];
        assert!(
            body.contains("ensure_source_worker_client_still_current("),
            "scoped rescan acceptance must fail closed if the accepted worker client is no longer current: {func}"
        );
    }
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn worker_targeted_rescan_request_rejects_without_local_primary_scan_root() {
    let boundary = Arc::new(LoopbackWorkerBoundary::default());
    let state_boundary = in_memory_state_boundary();
    let worker_socket_dir = worker_socket_tempdir();
    let factory =
    ...
    let actual = err.into_error();
    let CnxError::InvalidInput(message) = actual else {
        panic!(
            "targeted rescan without local primary root should be invalid input, got {actual:?}"
        );
    };
    assert!(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn worker_targeted_rescan_request_returns_pending_while_retained_replay_required() {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    let err = tokio::time::timeout(
        Duration::from_millis(250),
        client.submit_targeted_rescan_request_epoch_with_failure(),
    )
    .await
    .expect("scoped rescan submit must fail fast while retained replay is required")
    .expect_err("scoped rescan must be pending while retained source replay is required")
    .into_error();
    let CnxError::NotReady(message) = err else {
        panic!("retained replay should return retryable not-ready evidence, got {err:?}");
    };
    assert!(
        message.contains("retained control state replays"),
        "pending reason should name retained source replay: {message}"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn worker_targeted_rescan_delivery_check_does_not_queue_behind_control_apply() {
    let _control_reset = SourceWorkerControlFramePauseHookReset;
    let _accept_reset = SourceWorkerAcceptTargetedDeliveryCallCountHookReset;
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    let err = tokio::time::timeout(
        ...
        client.check_targeted_rescan_delivery_acceptance_with_failure(),
    )
    .await
    .expect("delivery acceptance check must not queue behind an applying control frame")
    .expect_err("delivery acceptance check should report not-ready while control applies")
    .into_error();
    let CnxError::NotReady(message) = err else {
        panic!("delivery acceptance check should fail-fast with not-ready, got {err:?}");
    };
    assert!(
        message.contains("control state is applying"),
        "not-ready message should name the applying control state: {message}"
    );
    assert_eq!(
        accept_rpc_count.load(Ordering::Relaxed),
        0,
        "delivery acceptance check must not enqueue a worker RPC behind active control apply"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn worker_targeted_rescan_submit_with_acceptance_timeout_waits_for_control_apply() {
    let _control_reset = SourceWorkerControlFramePauseHookReset;
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    assert!(
        ...
        "acceptance-timeout submit should wait inside its budget while control apply is still inflight"
    );

    release.notify_waiters();

    let epoch = tokio::time::timeout(Duration::from_secs(1), submit_task)
        .await
        .expect("targeted submit should finish once control apply releases")
        .expect("join targeted submit task")
        .expect("targeted submit should succeed within acceptance budget");
    assert!(
        epoch > 0,
        "targeted submit should return a recorded rescan epoch"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn worker_targeted_rescan_submit_with_acceptance_timeout_returns_timeout_when_budget_expires()
{
    let _delay_reset = SourceWorkerSubmitTargetedRescanDelayHookReset;
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    install_source_worker_submit_targeted_rescan_delay_hook(
        SourceWorkerSubmitTargetedRescanDelayHook {
    ...
    let err = tokio::time::timeout(
        ...
        client.submit_targeted_rescan_request_epoch_with_acceptance_timeout_with_failure(
            Duration::from_millis(100),
        ),
    )
    .await
    .expect("targeted submit should fail within caller timeout")
    .expect_err("targeted submit should time out when acceptance budget expires")
    .into_error();
    assert!(
        matches!(err, CnxError::Timeout),
        "expired acceptance budget should surface timeout, got {err:?}"
    );
...
#[test]
fn source_worker_when_ready_requests_submit_intent_without_waiting_for_scan_readiness() {
    let source_server_impl = include_str!("../source_server.rs");
    assert!(
        source_server_impl.contains(
            "SourceWorkerAction::TriggerRescanWhenReadyEpoch { source } => {\n            match source.submit_rescan_request_epoch_with_failure()"
        ),
        "source worker trigger-when-ready RPC must record a rescan intent without waiting for scan-root readiness inside the worker RPC session"
    );
    assert!(
        source_server_impl.contains(
            "SourceWorkerAction::TriggerTargetedRescanWhenReadyEpoch { source } => {\n            match source.submit_targeted_rescan_request_epoch_with_failure()"
        ),
        "source worker targeted trigger-when-ready RPC must record target-local rescan intent without waiting for scan-root readiness inside the worker RPC session"
    );
    assert!(
        !source_server_impl.contains(
            "SourceWorkerAction::TriggerRescanWhenReadyEpoch { source } => {\n            match source.trigger_rescan_when_ready_epoch_with_failure().await"
        ),
        "source worker trigger-when-ready RPC must not hold the RPC session while waiting for scan-root readiness"
    );
    assert!(
        !source_server_impl.contains(
            "SourceWorkerAction::TriggerTargetedRescanWhenReadyEpoch { source } => {\n            match source.targeted_rescan_when_ready_epoch_with_failure().await"
        ),
        "source worker targeted trigger-when-ready RPC must not hold the RPC session while waiting for scan-root readiness"
    );
}
...
struct SourceWorkerSubmitTargetedRescanDelayHookReset;

impl Drop for SourceWorkerSubmitTargetedRescanDelayHookReset {
    fn drop(&mut self) {
        clear_source_worker_submit_targeted_rescan_delay_hook();
    }
}
...
struct SourceWorkerReplacementShutdownHookReset;

impl Drop for SourceWorkerReplacementShutdownHookReset {
    fn drop(&mut self) {
        clear_source_worker_replacement_shutdown_hook();
    }
}
...
#[test]
fn prime_cached_schedule_from_control_signals_treats_source_unit_as_source_side_owner() {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    ...
    assert_eq!(
        ...
        "source-side runtime ownership covers roots that need scan/audit even when realtime watch is disabled"
    );
    assert_eq!(
...
fn replay_recovery_cached_schedule_primes_desired_local_groups_from_grants() {
    ...
    assert_eq!(
        ...
        "source-side ownership priming must include scan-capable roots even when watch is disabled"
    );
    ...
    assert_eq!(
        ...
        "replay-recovery bridge must include local scan-capable roots in source-side groups"
    );
    assert_eq!(
...
fn degraded_worker_observability_uses_cached_snapshot() {
    let cache = SourceWorkerSnapshotCache {
        ...
        runtime_scope_control_cache_bootstrap_served: false,
    };
...
fn source_control_state_tick_fast_path_carries_retained_scoped_source_route_activation() {
    ...
    let scan_tick =
        source_control_state_test_tick_signal(SourceRuntimeUnit::Scan, &scan_route, 1777494559668);
...
async fn rearm_source_rescan_endpoints_replays_retained_control_before_ready_proof() {
    ...
    let ready = format!(
        "ready unit=runtime.exec.source route={scoped_route} generation=1777519455077 scopes=[\"nfs1=>nfs1\"]"
    );
    assert!(
        snapshot
            .last_control_frame_signals_by_node
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn source_repair_retained_replay_cancellation_preserves_replay_required() {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    let client = Arc::new(
        SourceWorkerClientHandle::new(
            NodeId("node-b-retained-replay-cancel".to_string()),
            cfg,
            external_source_worker_binding(worker_socket_dir.path()),
            factory,
        )
    ...
    tokio::time::timeout(
        SOURCE_WORKER_CONTROL_TOTAL_TIMEOUT + Duration::from_secs(5),
        client.start(),
    )
    .await
    .expect("source worker start timed out")
    ...
    let retained_control = vec![
        encode_runtime_exec_control(&RuntimeExecControl::Activate(RuntimeExecActivate {
            route_key: ROUTE_KEY_QUERY.to_string(),
            unit_id: SOURCE_RUNTIME_UNIT_ID.to_string(),
            lease: None,
            generation: 1777519455077,
            expires_at_ms: 1,
            bound_scopes: vec![bound_scope_with_resources("nfs1", &["node-b::nfs1"])],
        }))
        .expect("encode retained source activate"),
    ];
    let retained_signals = source_control_signals_from_envelopes(&retained_control)
        .expect("decode retained source activate");
    client.retain_control_signals(&retained_signals).await;
    client.control_state.lock().await.arm_replay();
    ...
    let replay = tokio::spawn({
        ...
        async move {
            client
                .replay_retained_control_state_for_source_repair_with_failure()
                .await
        }
    });
    tokio::time::timeout(Duration::from_secs(2), entered.notified())
        .await
        .expect("retained replay control-frame attempt should enter paused worker RPC");
    ...
    assert_eq!(
        ...
        SourceControlReplayState::Required,
        "cancelling a bounded source-repair retained replay must preserve replay-required so later recovery can retry instead of falsely reopening readiness"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn reconnect_shared_source_worker_does_not_publish_successor_until_stale_shutdown_finishes() {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    let entered = Arc::new(Notify::new());
    let release = Arc::new(Notify::new());
    let _reset = SourceWorkerReplacementShutdownHookReset;
    install_source_worker_replacement_shutdown_hook(SourceWorkerReplacementShutdownHook {
        entered: entered.clone(),
        release: release.clone(),
    });
    ...
    let reconnect = tokio::spawn({
        let client = client.clone();
        async move { client.reconnect_shared_worker_client_with_failure().await }
    });
    tokio::time::timeout(Duration::from_secs(2), entered.notified())
        .await
        .expect("stale source worker shutdown should start");

    match tokio::time::timeout(
        Duration::from_millis(150),
        client.worker_instance_id_for_tests(),
    )
    .await
    {
        Ok(observed_instance_id) => assert_eq!(
            observed_instance_id, previous_instance_id,
            "source shared-worker replacement must not publish a successor instance while stale shutdown is still in progress",
        ),
        Err(_) => {}
    }

    release.notify_waiters();
    tokio::time::timeout(Duration::from_secs(4), reconnect)
        .await
        .expect("source shared-worker replacement should finish after stale shutdown release")
        .expect("join source shared-worker replacement")
        .expect("source shared-worker replacement should succeed");

    assert_ne!(
        client.worker_instance_id_for_tests().await,
        previous_instance_id,
        "source shared-worker replacement should publish the successor after stale shutdown completes",
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn reconnect_shared_source_worker_fails_closed_when_stale_shutdown_does_not_finish() {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    let client = Arc::new(
        SourceWorkerClientHandle::new(
            NodeId("node-d-stale-shutdown-timeout".to_string()),
            cfg,
            external_source_worker_binding(worker_socket_dir.path()),
            factory,
        )
    ...
    let replacement = tokio::spawn({
        ...
        async move {
            client
                .replace_shared_worker_client_after_stale_shutdown(
    ...
    tokio::time::timeout(Duration::from_secs(2), entered.notified())
        .await
        .expect("stale source worker shutdown should have been attempted");
    let result = tokio::time::timeout(Duration::from_secs(2), replacement)
        .await
        .expect("replacement fail-closed result should stay bounded")
        .expect("replacement task should not panic");
    let err = result.expect_err("replacement must fail closed when stale shutdown cannot finish");
    assert!(
        err.to_string()
            .contains("stale source worker shutdown timed out"),
        "stale shutdown timeout should surface at the raw replacement boundary: {err}",
    );
    assert_eq!(
        client.worker_instance_id_for_tests().await,
        previous_instance_id,
        "source shared-worker replacement must not publish a successor when stale shutdown does not finish",
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn rootless_source_worker_replacement_bootstraps_from_current_logical_roots() {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    let nfs2 = tmp.path().join("nfs2");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    let client = Arc::new(
        SourceWorkerClientHandle::new(
            NodeId("node-d-rootless-replacement".to_string()),
            cfg,
            external_source_worker_binding(worker_socket_dir.path()),
            factory,
        )
    ...
    tokio::time::timeout(Duration::from_secs(8), client.start())
        .await
        .expect("source worker start timed out")
        .expect("start rootless source worker");

    client
        .update_logical_roots_with_failure(current_roots.clone())
        .await
        .expect("update current logical roots before replacement");

    let before_replacement = client
        .logical_roots_snapshot_with_failure()
        .await
        .expect("logical roots after current update");
    assert_eq!(
        before_replacement
            .iter()
            .map(|root| root.id.as_str())
            .collect::<Vec<_>>(),
        ["nfs1", "nfs2"],
        "rootless source worker should accept current logical roots before replacement",
    );
    ...
    client
        .reconnect_shared_worker_client_with_failure()
        .await
        .expect("replace rootless-origin source worker");
    assert_ne!(
        client.worker_instance_id_for_tests().await,
        previous_instance_id,
        "test should exercise a fresh shared source worker successor",
    );

    tokio::time::timeout(Duration::from_secs(8), client.start())
        .await
        .expect("successor source worker start timed out")
        .expect("start replacement source worker");

    let after_replacement = client
        .logical_roots_snapshot_with_failure()
        .await
        .expect("logical roots after replacement");
    assert_eq!(
        after_replacement
            .iter()
            .map(|root| root.id.as_str())
            .collect::<Vec<_>>(),
        ["nfs1", "nfs2"],
        "source shared-worker replacement must initialize successors from current logical roots, not the original roots=[] construction config",
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn generation_two_channel_closed_recovery_keeps_scoped_manual_rescan_endpoint_worker_owned() {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    client
        ...
        .await
        .expect(
            "generation-two recovery should record worker-owned manual-rescan delivery readiness",
        );
    let (snapshot, used_cached_fallback) = client
        .manual_rescan_delivery_observability_snapshot_for_status_route_with_timeout(Some(
            Duration::from_millis(20),
        ))
        .await;
    assert!(
        used_cached_fallback,
        "worker-backed scoped manual-rescan readiness must be exposed through delivery cache"
    );
    let ready = format!(
        "ready unit=runtime.exec.source route={local_rescan_route} generation=2 scopes=[\"nfs1=>nfs1\"]"
    );
    assert!(
        snapshot
            .last_control_frame_signals_by_node
            .values()
            .any(|signals| signals.iter().any(|signal| signal == &ready)),
        "generation-two recovery must publish current worker-owned manual-rescan readiness: expected={ready} control={:?}",
        snapshot.last_control_frame_signals_by_node
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn rearm_source_rescan_endpoint_via_node_does_not_claim_app_proxy_route() {
    let unique = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .expect("clock")
        .as_micros();
    ...
    let direct_worker_route_result = capanix_host_adapter_fs::HostAdapter::call_collect(
        &adapter,
    ...
    assert!(
        direct_worker_route_result.is_err(),
        "routed worker rearm must not expose the externally addressed scoped manual-rescan route from the source worker; app runtime owns that proxy route"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn management_status_runtime_scope_control_cache_does_not_wait_for_live_publication_probe() {
    let _observability_hook_guard = source_worker_observability_hook_test_guard().await;
    let _delay_hook_reset = SourceWorkerObservabilityDelayHookReset;
    let _count_hook_reset = SourceWorkerObservabilityCallCountHookReset;
    let tmp = tempdir().expect("create temp dir");
    ...
    assert!(
        ...
        "precondition: source/scan control ACK should be current"
    );
    ...
    .await
    .expect(
        "management status must not spend its source-control window on live publication probing",
    );

    assert!(
        used_cached_fallback,
        "source-control status should use acknowledged runtime-scope ownership as cache-provenance evidence"
    );
    assert_eq!(
        observability_rpc_count.load(Ordering::Relaxed),
        0,
        "source-control status must not call live source observability before exposing current ownership"
    );
    assert_eq!(
        snapshot.scheduled_source_groups_by_node.get("node-a"),
        Some(&vec!["nfs1".to_string(), "nfs2".to_string()])
    ...
    assert!(
        ...
        "runtime-scope status must be labelled as control-cache evidence: {:?}",
        snapshot.status.degraded_roots
    );
    assert!(
        snapshot.published_batches_by_node.is_empty()
            && snapshot.published_events_by_node.is_empty(),
        "runtime-scope control-cache status must not synthesize publication truth"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn nonblocking_status_observability_uses_control_cache_when_runtime_scope_lacks_publication()
{
    let _observability_hook_guard = source_worker_observability_hook_test_guard().await;
    let _count_hook_reset = SourceWorkerObservabilityCallCountHookReset;
    let tmp = tempdir().expect("create temp dir");
    ...
    assert!(
        used_cached_fallback,
        "source-status should expose current runtime-scope owners from control cache when publication evidence is not yet available"
    );
    assert_eq!(
        observability_rpc_count.load(Ordering::Relaxed),
        0,
        "runtime-scope owners without publication evidence must not be blocked on a live worker probe"
    );
    assert_eq!(
        nonblocking.scheduled_source_groups_by_node.get("node-a"),
        Some(&vec!["nfs1".to_string(), "nfs2".to_string()]),
        "source-status must preserve source runtime-scope evidence"
    );
    assert!(
        nonblocking
            .status
        ...
        "runtime-scope fallback must keep explicit control-cache provenance: {:?}",
...
fn source_observability_override_preserves_live_scoped_rescan_route_evidence() {
    ...
    let live_summary = std::collections::BTreeMap::from([(
        ...
        vec![
            ...
            format!(
                "ready unit=runtime.exec.source route={route} generation=1777508542667 scopes=[\"nfs2=>nfs2\"]"
            ),
        ],
    )]);
    ...
    assert!(
        signals.iter().any(|signal| signal
            == &format!("ready unit=runtime.exec.source route={route} generation=1777508542667 scopes=[\"nfs2=>nfs2\"]")),
        "worker observability override must preserve the source-owned scoped manual-rescan ready proof: {signals:?}"
    );
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn nonblocking_status_observability_uses_runtime_scope_control_cache_when_replay_is_current()
{
    let _observability_hook_guard = source_worker_observability_hook_test_guard().await;
    let _count_hook_reset = SourceWorkerObservabilityCallCountHookReset;
    let tmp = tempdir().expect("create temp dir");
    ...
    assert!(
        used_cached_fallback,
        "current replay with acknowledged source/scan ownership should expose runtime-scope control evidence without waiting for live publication"
    );
    assert_eq!(
        observability_rpc_count.load(Ordering::Relaxed),
        0,
        "source-status should not issue a live worker RPC before exposing current runtime-scope ownership"
    );
    assert_eq!(
        snapshot.scheduled_source_groups_by_node.get("node-b"),
        Some(&vec!["nfs1".to_string()])
    ...
    assert!(
        ...
        "runtime-scope control evidence must be labelled as cache provenance: {:?}",
        snapshot.status.degraded_roots
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn nonblocking_status_observability_live_probes_after_runtime_scope_bootstrap_cache_is_used()
{
    let _observability_hook_guard = source_worker_observability_hook_test_guard().await;
    let _count_hook_reset = SourceWorkerObservabilityCallCountHookReset;
    let tmp = tempdir().expect("create temp dir");
    ...
    let (bootstrap, bootstrap_used_cached_fallback) = client
        .observability_snapshot_nonblocking_for_status_route()
        .await;

    assert!(
        bootstrap_used_cached_fallback,
        "first source-status read may expose acknowledged runtime-scope ownership before live root health is available"
    );
    assert_eq!(
        observability_rpc_count.load(Ordering::Relaxed),
        0,
        "bootstrap read should not spend its status window on live publication probing"
    );
    assert_eq!(
        bootstrap.scheduled_source_groups_by_node.get("node-b"),
        Some(&vec!["nfs1".to_string()])
    );

    let (live, live_used_cached_fallback) = client
        .observability_snapshot_nonblocking_for_status_route()
        .await;

    assert!(
        !live_used_cached_fallback,
        "after the ownership bootstrap has been served once, empty control-cache status must not keep hiding live source root health"
    );
    assert_eq!(
        observability_rpc_count.load(Ordering::Relaxed),
        1,
        "second source-status read must issue one live observability probe"
    );
    assert!(
        live.status
            .concrete_roots
            .iter()
            .any(|root| root.logical_root_id == "nfs1" && root.scan_enabled),
        "live source-status should expose concrete source/scan root health after the worker has accepted the scope: {:?}",
        live.status.concrete_roots
    );
    assert!(
        !live.status.degraded_roots.iter().any(|(root_key, reason)| {
            root_key == SOURCE_WORKER_DEGRADED_ROOT_KEY
                && reason == SOURCE_WORKER_RUNTIME_SCOPE_CACHE_REASON
        }),
        "live source-status must not keep runtime-scope cache provenance: {:?}",
        live.status.degraded_roots
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn nonblocking_status_observability_keeps_runtime_scope_without_live_probe_when_control_cache_is_current()
 {
    let _observability_hook_guard = source_worker_observability_hook_test_guard().await;
    let _count_hook_reset = SourceWorkerObservabilityCallCountHookReset;
    let tmp = tempdir().expect("create temp dir");
    ...
    assert_eq!(
        observability_rpc_count.load(Ordering::Relaxed),
        0,
        "source-status should not need a failed live probe before using runtime-scope ownership evidence"
    );
    assert_eq!(
        snapshot.scheduled_source_groups_by_node.get("node-b"),
        Some(&vec!["nfs1".to_string()])
...
async fn nonblocking_status_observability_times_out_live_probe_inside_route_budget() {
    ...
    assert!(
        used_cached_fallback,
        "source-status should return bounded degraded status when no runtime-scope cache exists and the live probe exceeds its route budget"
    );
    assert_eq!(
        observability_rpc_count.load(Ordering::Relaxed),
        1,
    ...
    assert!(
        source_observability_snapshot_is_degraded_worker_cache(&snapshot),
        "when no current runtime-scope cache exists, a live-probe timeout should be surfaced as degraded worker evidence: {:?}",
        snapshot.status.degraded_roots
    );
...
async fn manual_rescan_delivery_status_uses_rearm_ack_cache_without_live_probe() {
    ...
    assert!(
        used_cached_fallback,
        "manual-rescan delivery status should use the app-route cache as delivery proof"
    );
    assert_eq!(
        observability_rpc_count.load(Ordering::Relaxed),
        0,
    ...
    assert!(
        ...
        "app-route snapshot should carry current logical-root delivery readiness: {:?}",
    ...
    assert!(
        ...
        "app-route snapshot should carry local source-primary concrete-root delivery readiness: {:?}",
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn manual_rescan_app_route_proof_uses_control_cache_without_live_probe() {
    ...
    let snapshot = tokio::time::timeout(
        Duration::from_millis(150),
        client.manual_rescan_app_route_observability_snapshot_for_status_route(None),
    )
    .await
    .expect("app-route manual-rescan status proof must not wait for live source observability")
    .expect("app-route manual-rescan status proof should be available from control cache");

    assert_eq!(
        observability_rpc_count.load(Ordering::Relaxed),
        0,
        "app-route manual-rescan proof must not call live observability or wait on audit"
    );
    assert!(
        snapshot.status.logical_roots.iter().any(|root| {
            root.root_id == "nfs1" && root.status == "manual_rescan_delivery_ready"
    ...
    let inflight = client.begin_control_op();
    let applying_snapshot = tokio::time::timeout(
        Duration::from_millis(150),
        client.manual_rescan_app_route_observability_snapshot_for_status_route(None),
    )
    .await
    .expect("control-applying app-route observation must not wait for live source observability");
    assert!(
        applying_snapshot.is_none(),
        "app-route cache is not manual-rescan target proof while source control state is applying"
    );
    drop(inflight);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn manual_rescan_app_route_proof_declares_only_delivered_roots() {
    let _observability_count_reset = SourceWorkerObservabilityCallCountHookReset;
    let _observability_delay_reset = SourceWorkerObservabilityDelayHookReset;
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    ...
    let node_id = NodeId("node-b".to_string());
    let nfs1_root = worker_watch_scan_root("nfs1", &nfs1);
    let nfs2_root = worker_watch_scan_root("nfs2", &nfs2);
    let nfs1_grant = worker_source_export("node-b::nfs1", "node-b", "10.0.0.21", nfs1.clone());
    let nfs2_grant = worker_source_export("node-b::nfs2", "node-b", "10.0.0.21", nfs2.clone());
    let cfg = SourceConfig {
        roots: vec![nfs1_root.clone(), nfs2_root.clone()],
        host_object_grants: vec![nfs1_grant.clone(), nfs2_grant.clone()],
        ..SourceConfig::default()
    };
    let boundary = Arc::new(LoopbackWorkerBoundary::default());
    let state_boundary = in_memory_state_boundary();
    ...
    client.with_cache_mut(|cache| {
        cache.logical_roots = Some(vec![nfs1_root.clone(), nfs2_root.clone()]);
        cache.grants = Some(vec![nfs1_grant.clone(), nfs2_grant.clone()]);
        prime_cached_schedule_from_control_signals(
            cache,
            &node_id,
            &signals,
    ...
    let delivery_route_groups = std::collections::BTreeSet::from(["nfs1".to_string()]);
    let snapshot = client
        .manual_rescan_app_route_observability_snapshot_for_status_route(Some(
            &delivery_route_groups,
        ))
        .await
    ...
    assert_eq!(
        snapshot
            ...
            .collect::<Vec<_>>(),
        vec!["nfs1"],
        "manual-rescan delivery proof must not keep stale non-delivered roots in its root declaration"
    );
    assert_eq!(
        snapshot
            .status
            ...
            .collect::<Vec<_>>(),
        vec!["nfs1"],
        "manual-rescan delivery health must stay aligned with the declared delivered roots"
    );
    assert_eq!(
        snapshot
            .scheduled_source_groups_by_node
            ...
            .unwrap_or_default(),
        vec!["nfs1"],
        "manual-rescan delivery proof must not keep stale source schedule groups"
    );
    assert_eq!(
        snapshot
            ...
            .unwrap_or_default(),
        vec!["nfs1"],
        "manual-rescan delivery proof must not keep stale scan schedule groups"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn manual_rescan_app_route_proof_rebuilds_schedule_from_current_roots_after_restore() {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    let nfs2 = tmp.path().join("nfs2");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    client.with_cache_mut(|cache| {
        cache.logical_roots = Some(vec![nfs1_root.clone()]);
        cache.grants = Some(vec![nfs1_grant.clone()]);
        cache.scheduled_source_groups_by_node = Some(std::collections::BTreeMap::from([(
            "node-b".to_string(),
            vec!["nfs1".to_string()],
        )]));
    ...
    let snapshot = client
        .manual_rescan_app_route_observability_snapshot_for_status_route(None)
        .await
        .expect("app-route proof should rebuild from current config and retained control");
    ...
    assert_eq!(
        snapshot
            ...
            .collect::<Vec<_>>(),
        vec!["nfs1", "nfs2"],
        "manual-rescan proof must use current app-authoritative roots after roots restore"
    );
    assert_eq!(
        snapshot
            .scheduled_scan_groups_by_node
            ...
            .unwrap_or_default(),
        vec!["nfs1".to_string(), "nfs2".to_string()],
        "manual-rescan proof must not derive local schedule from stale cache roots"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn retained_activate_control_signals_prime_runtime_scope_cache_before_worker_ack() {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    let nfs2 = tmp.path().join("nfs2");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    let envelopes = vec![
        ...
        .expect("encode retained scan activate"),
    ];
    let signals = source_control_signals_from_envelopes(&envelopes)
        .expect("decode retained source control signals");
    ...
    assert!(
        ...
        "precondition: retained source replay must still be pending before worker ACK"
    );

    let (snapshot, used_cached_fallback) = client
        .observability_snapshot_nonblocking_for_status_route_with_timeout(Some(
            Duration::from_millis(1),
        ))
        .await;
    ...
    assert!(
        used_cached_fallback,
        "source-status must expose retained runtime-scope control cache before worker ACK/replay"
    );
    assert_eq!(
        snapshot.scheduled_source_groups_by_node.get("node-a"),
        Some(&vec!["nfs1".to_string(), "nfs2".to_string()]),
        "retained source activation should prime source ownership"
    );
    assert_eq!(
        snapshot.scheduled_scan_groups_by_node.get("node-a"),
        Some(&vec!["nfs1".to_string(), "nfs2".to_string()]),
        "retained scan activation should prime scan ownership"
    );
    assert!(
        snapshot
            .status
        ...
        "retained runtime-scope evidence must be labelled as control-cache provenance: {:?}",
        snapshot.status.degraded_roots
    );
    assert!(
        snapshot.status.logical_roots.is_empty() && snapshot.status.concrete_roots.is_empty(),
        "retained control cache must not fabricate source health before live worker ACK: {:?}",
        snapshot.status
    );
}
...
async fn on_control_frame_does_not_exhaust_post_ack_schedule_refresh_when_mixed_cluster_grants_leave_no_local_runnable_groups()
 {
    ...
    assert!(
        observability
            ...
            .values()
            .all(Vec::is_empty),
        "node-e should preserve a source schedule with no runnable groups when the mixed-cluster wave carries only nfs1/nfs2 and all refreshed scheduled groups are legitimately empty for the local node: {:?}",
        observability.scheduled_source_groups_by_node
    );
    assert!(
        observability
            .scheduled_scan_groups_by_node
            .values()
            .all(Vec::is_empty),
        "node-e should preserve a scan schedule with no runnable groups when the mixed-cluster wave carries only nfs1/nfs2 and all refreshed scheduled groups are legitimately empty for the local node: {:?}",
        observability.scheduled_scan_groups_by_node
    );
