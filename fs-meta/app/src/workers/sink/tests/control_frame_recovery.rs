...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn retained_sink_control_replay_keeps_recovery_rpc_budget_for_existing_client() {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    tokio::time::timeout(Duration::from_secs(1), entered.notified())
        .await
        .expect("retained control-frame rpc did not enter pause hook");
    tokio::time::sleep(Duration::from_millis(2500)).await;
    release.notify_waiters();

    apply
        .await
        .expect("join retained control replay")
        .expect("retained sink control replay must not use the short stale-client budget");
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn active_sink_control_apply_keeps_standard_rpc_budget_for_existing_client() {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    std::fs::create_dir_all(&nfs1).expect("create nfs1 dir");
    ...
    let initial_envelope =
        encode_runtime_exec_control(&RuntimeExecControl::Activate(RuntimeExecActivate {
            ...
            unit_id: "runtime.exec.sink".to_string(),
            lease: None,
            generation: 1_777_634_407_000,
            expires_at_ms: 1,
            bound_scopes: vec![bound_scope_with_resources("nfs1", &["node-d::nfs1"])],
        }))
        .expect("encode initial sink activate");
    sink.on_control_frame(vec![initial_envelope])
        .await
        .expect("seed existing active sink control state");
    ...
    let facade = SinkFacade::Worker(sink.clone());
    let apply = tokio::spawn(async move {
        facade
            .apply_orchestration_signals_with_total_timeout_with_failure(
                &signals,
    ...
    tokio::time::timeout(Duration::from_secs(1), entered.notified())
        .await
        .expect("active control-frame rpc did not enter pause hook");
    tokio::time::sleep(Duration::from_millis(2500)).await;
    release.notify_waiters();

    apply
        .await
        .expect("join active control apply")
        .expect("active sink control apply must not use the short stale-client budget");
}
