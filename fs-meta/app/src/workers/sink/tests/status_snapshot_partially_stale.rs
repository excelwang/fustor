...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn status_snapshot_nonblocking_fails_closed_while_actual_second_wave_sink_apply_is_paused_even_if_live_snapshot_still_looks_ready()
 {
    ...
    .await
    .expect("apply initial sink control before paused second wave");
    ...
    tokio::time::timeout(Duration::from_secs(5), entered.notified())
        .await
        .expect("second-wave sink apply should reach the pause point");

    let err = sink
        .status_snapshot_nonblocking()
        .await
        .expect_err(
            "status_snapshot_nonblocking must fail closed while an actual second-wave sink.apply is paused, even if the stale live snapshot still looks ready",
        );
    assert!(
        matches!(err, CnxError::Timeout),
        "paused second-wave sink.apply should fail closed instead of surfacing the stale ready snapshot: err={err:?}"
    );

    release.notify_waiters();
    inflight_control
        .await
        .expect("join paused second-wave sink control")
        .expect("paused second-wave sink control should finish after release");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn status_snapshot_nonblocking_fails_closed_for_control_inflight_live_mixed_ready_and_unready_when_cached_ready_groups_do_not_cover_live_schedule()
 {
    let tmp = tempdir().expect("create temp dir");
    let nfs1 = tmp.path().join("nfs1");
    let nfs2 = tmp.path().join("nfs2");
    ...
    sink.update_cached_status_snapshot(cached_snapshot.clone())
        .expect("seed unrelated ready cached snapshot");
    ...
    let err = sink
        ...
        .await
        .expect_err(
            "status_snapshot_nonblocking must fail closed when control-inflight live mixed ready/unready does not match the cached ready schedule",
        );

    assert!(
        matches!(err, CnxError::Timeout),
        "status_snapshot_nonblocking must fail closed instead of returning unrelated cached ready groups when live schedule changed under control inflight: cached={cached_snapshot:?} live={live_snapshot:?} err={err:?}"
    );
