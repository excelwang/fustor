...
use capanix_host_adapter_fs::{HostAdapter, PostBindDispatchTable};
...
impl StatusRouteMachine {
    ...
    async fn collect_status_events(
        ...
        self.collect_status_events_with_routes(
            boundary,
            origin_id,
            method,
    ...
    async fn collect_status_events_with_routes(
        self,
        boundary: Arc<dyn ChannelIoSubset>,
        origin_id: NodeId,
        method: &'static str,
...
impl Default for MaterializedStatusLoadPlan {
    fn default() -> Self {
        Self {
            ...
            owner_scoped_sink_route: StatusRoutePlan::new(
                Duration::from_secs(30),
                STATUS_ROUTE_COLLECT_IDLE_GRACE,
            ),
...
pub(crate) async fn route_sink_status_snapshot(
    ...
    let events = plan
        ...
        .await?;
    merge_sink_status_route_events(events)
}

async fn route_sink_status_snapshot_via_node(
    boundary: Arc<dyn ChannelIoSubset>,
    origin_id: NodeId,
    node_id: NodeId,
    plan: StatusRoutePlan,
) -> Result<SinkStatusSnapshot, CnxError> {
    let route_bindings = sink_status_route_bindings_for(&node_id.0);
    let events = plan
        .machine()
...
fn merge_sink_status_route_events(events: Vec<Event>) -> Result<SinkStatusSnapshot, CnxError> {
    let snapshots = events
        .into_iter()
        .map(|event| {
...
fn missing_readiness_groups(
    readiness_groups: &BTreeSet<String>,
    observed_groups: &BTreeSet<String>,
) -> BTreeSet<String> {
    readiness_groups
...
fn readiness_group_list(groups: &BTreeSet<String>) -> String {
    groups.iter().cloned().collect::<Vec<_>>().join(",")
}

fn trusted_materialized_root_readiness_gap_message(
    path: &[u8],
    selected_group: Option<&str>,
    expected_readiness_groups: Option<&BTreeSet<String>>,
    source_status: &SourceStatusSnapshot,
    sink_status: &SinkStatusSnapshot,
) -> Option<String> {
    if selected_group.is_some() || !trusted_materialized_empty_group_root_requires_fail_closed(path)
    {
        return None;
    }
    let source_logical_groups = source_status
        .logical_roots
        .iter()
        .map(|root| root.root_id.clone())
        .collect::<BTreeSet<_>>();
    let readiness_groups = expected_readiness_groups
        .filter(|groups| !groups.is_empty())
        .cloned()
        .unwrap_or_else(|| source_logical_groups.clone());
    if readiness_groups.is_empty() {
        return Some(
            "trusted-materialized reads remain unavailable until root materialized readiness has active scan groups"
                .to_string(),
        );
    }

    let mut source_groups = source_logical_groups;
    ...
    let ready_groups = sink_status
        ...
        .collect::<BTreeSet<_>>();
    let overflow_pending_groups = sink_status
        .groups
        .iter()
    ...
    let degraded_groups = source_status
        ...
        .filter(|group| readiness_groups.contains(group))
        .collect::<BTreeSet<_>>();

    let missing_source = missing_readiness_groups(&readiness_groups, &source_groups);
    let missing_schedule = missing_readiness_groups(&readiness_groups, &scheduled_groups);
    let missing_ready = missing_readiness_groups(&readiness_groups, &ready_groups);
    let overflow_pending = readiness_groups
    ...
    if !missing_source.is_empty() {
        reasons.push(format!(
            ...
            readiness_group_list(&missing_source)
        ));
    }
    if !missing_schedule.is_empty() {
        reasons.push(format!(
            "sink schedule missing [{}]",
            readiness_group_list(&missing_schedule)
        ));
    }
    if !missing_ready.is_empty() {
        reasons.push(format!(
            "sink readiness missing [{}]",
            readiness_group_list(&missing_ready)
        ));
    }
    ...
    if !degraded_groups.is_empty() {
        reasons.push(format!(
            "degraded coverage [{}]",
            readiness_group_list(&degraded_groups)
        ));
    }

    Some(format!(
        "trusted-materialized reads remain unavailable until root materialized readiness covers all active scan groups: {}",
        reasons.join("; ")
    ))
}
...
fn cache_trusted_materialized_ready_status_snapshots(
    state: &ApiState,
    readiness_groups: &BTreeSet<String>,
    source_status: SourceStatusSnapshot,
    sink_status: SinkStatusSnapshot,
...
fn materialized_readiness_groups_for_loaded_source(
    fallback_groups: &BTreeSet<String>,
    source_status: &SourceStatusSnapshot,
) -> BTreeSet<String> {
    let mut groups = fallback_groups.clone();
    groups.extend(
        source_status
            .logical_roots
            .iter()
...
fn missing_sink_ready_readiness_groups(
    sink_status: &SinkStatusSnapshot,
    readiness_groups: &BTreeSet<String>,
) -> BTreeSet<String> {
    let scheduled_groups = materialized_scheduled_group_ids(sink_status);
    ...
    readiness_groups
        .iter()
        .filter(|group| !scheduled_groups.contains(*group) || !ready_groups.contains(*group))
        .cloned()
        .collect()
}
...
fn current_owner_candidate_nodes_for_groups(
    ...
    for root in roots
        .into_iter()
        .filter(|root| root.scan && groups.contains(&root.id))
    {
        nodes.extend(
            grants
                .iter()
...
fn current_owner_candidate_nodes_for_groups_and_source_status(
    ...
    let mut observed_candidate_nodes =
        current_owner_candidate_nodes_from_source_status(source_status, groups);
    if let Some(sink_status) = sink_status {
        observed_candidate_nodes.extend(current_owner_candidate_nodes_from_sink_status(
            sink_status,
            groups,
        ));
    }
...
fn collect_nodes_for_groups_from_schedule(
    ...
    for (node_id, scheduled_groups) in schedule {
        if scheduled_groups.iter().any(|group| groups.contains(group)) {
            nodes.insert(node_id.clone());
        }
    }
}

fn sink_status_origin_count_entry_is_positive(entry: &str) -> bool {
    entry
        .rsplit_once('=')
...
fn collect_nodes_for_groups_from_origin_counts(
    nodes: &mut BTreeSet<String>,
    origin_counts_by_node: &BTreeMap<String, Vec<String>>,
    groups: &BTreeSet<String>,
) {
    for (node_id, origin_counts) in origin_counts_by_node {
        if node_id.trim().is_empty() {
            continue;
        }
        if origin_counts.iter().any(|entry| {
            groups.contains(crate::sink::sink_status_origin_entry_group_id(entry))
...
fn current_owner_candidate_nodes_from_sink_status(
    ...
    for (group_id, host_ref) in &sink_status.primary_host_ref_by_group {
        let host_ref = host_ref.trim();
        if groups.contains(group_id) && !host_ref.is_empty() {
            nodes.insert(host_ref.to_string());
        }
    }
    for origin_counts_by_node in [
        &sink_status.stream_applied_origin_counts_by_node,
        &sink_status.stream_applied_path_origin_counts_by_node,
        &sink_status.stream_ready_origin_counts_by_node,
        &sink_status.stream_ready_path_origin_counts_by_node,
        ...
        collect_nodes_for_groups_from_origin_counts(&mut nodes, origin_counts_by_node, groups);
    }
    nodes.into_iter().map(NodeId).collect()
}
...
async fn augment_routed_sink_status_with_current_owner_evidence(
    ...
    plan: StatusRoutePlan,
    source_observability_snapshots: &[SourceObservabilitySnapshot],
) -> std::result::Result<SinkStatusSnapshot, QueryWorkerObservationFailure> {
    if sink_status_covers_ready_readiness_groups(&sink_status, readiness_groups) {
    ...
    let mut pending = candidate_nodes
        ...
        .map(|node_id| {
            ...
            async move {
                let result =
                    route_sink_status_snapshot_via_node(boundary, origin_id, node_id.clone(), plan)
                        .await;
                (node_id, result)
            }
        })
    ...
    while let Some((_node_id, result)) = pending.next().await {
        ...
        let filtered = filter_sink_status_snapshot(snapshot, readiness_groups);
        sink_status = merge_sink_status_snapshots(vec![sink_status, filtered]);
        if sink_status_covers_ready_readiness_groups(&sink_status, readiness_groups) {
            break;
        }
    }
...
async fn load_materialized_status_snapshots_with_plan(
    ...
    let (mut source_status, mut sink_status) = match &state.backend {
        QueryBackend::Route {
            boundary,
            origin_id,
            ..
        } => {
            let source_status_load = route_source_observability_snapshots(
                boundary.clone(),
                origin_id.clone(),
            ...
            let source_status = match source_status_result {
                Ok(snapshots) => {
                    routed_source_observability_snapshots = snapshots;
                    merge_source_status_snapshots(
    ...
    if status_load_plan.require_trusted_cache
    ...
    {
        return cache_trusted_materialized_ready_status_snapshots(
            state,
            &readiness_groups,
            source_status,
            sink_status,
    ...
    let sink_owner_evidence_plan = if explicit_empty_all_active_recollect_attempted
        ...
        && current_owner_candidate_nodes_from_sink_status(&sink_status, &readiness_groups)
            .is_empty()
    {
        status_load_plan.explicit_empty_sink_recollect
    } else {
        status_load_plan.owner_scoped_sink_route
    };
...
async fn load_request_scoped_materialized_sink_status_snapshot(
    ...
    match &state.backend {
        QueryBackend::Route {
            ...
            let route_timeout = if cached_ready.is_some() && plan.use_ready_cache_fallback() {
                std::cmp::min(
                    plan.route_timeout(),
            ...
            .or_else(|| {
                plan.use_ready_cache_fallback()
                    .then_some(cached_ready)
                    .flatten()
            })
        }
...
fn preserve_loaded_ready_groups_for_request_scoped_service_gate(
    current: SinkStatusSnapshot,
    loaded_sink_status: &SinkStatusSnapshot,
) -> SinkStatusSnapshot {
    let current_by_group = current
...
fn materialized_ready_sink_evidence_key_for(group_id: &str, primary_object_ref: &str) -> String {
    format!("{group_id}\0{primary_object_ref}")
}
...
fn source_primary_object_refs_by_group(
    ...
    source_status
        .concrete_roots
        .iter()
        .filter(|root| root.active && root.scan_enabled && root.is_group_primary)
...
fn apply_materialized_ready_sink_evidence_cache(
    ...
    let source_primary_by_group = source_primary_object_refs_by_group(source_status);
    ...
    for group in &merged.groups {
        ...
        if !source_status_group_still_active(source_status, &group.group_id) {
            continue;
        }
        let cached = cache.get(&key).or_else(|| {
            source_primary_by_group
                .get(&group.group_id)
                .and_then(|primary| {
                    cache.get(&materialized_ready_sink_evidence_key_for(
                        &group.group_id,
                        primary,
                    ))
                })
    ...
    let observed_group_ids = merged
        .groups
        .iter()
        .map(|group| group.group_id.clone())
        .collect::<BTreeSet<_>>();
    let scheduled_groups = materialized_scheduled_group_ids(&merged);
    let restored_missing_groups = scheduled_groups
        .difference(&observed_group_ids)
        .filter(|group_id| source_status_group_still_active(source_status, group_id))
        .filter_map(|group_id| {
            let primary = source_primary_by_group.get(group_id)?;
            let cached = cache.get(&materialized_ready_sink_evidence_key_for(group_id, primary))?;
            sink_group_readiness_reports_live_materialized_group(cached).then(|| cached.clone())
        })
        .collect::<Vec<_>>();
    if replacements.is_empty() && restored_missing_groups.is_empty() {
        return merged;
    }
    ...
    merged.groups.extend(restored_missing_groups);
...
async fn route_materialized_events_via_node(
    ...
    if debug_materialized_route_capture_enabled() {
        let (entry_offset, entry_limit) = params
            .tree_options
            .as_ref()
            .map(|options| (options.entry_offset, options.entry_limit))
            .unwrap_or((0, None));
        eprintln!(
            "fs_meta_query_api: materialized_route_capture begin owner_node={} selected_group={:?} recursive={} path={} entry_offset={} entry_limit={:?} timeout_ms={}",
            owner_node,
            params.scope.selected_group,
            params.scope.recursive,
            String::from_utf8_lossy(&params.scope.path),
...
fn request_scoped_omitted_ready_root_group_requires_materialized_query(
    params: &InternalQueryRequest,
    snapshot: Option<&SinkStatusSnapshot>,
    request_scoped_schedule_omitted_ready_groups: Option<&BTreeSet<String>>,
    group_id: &str,
) -> bool {
    // Request-scoped sink-status omissions are fan-in evidence, not query content.
    // Broad trusted root reads must still obtain a bounded materialized page.
...
fn selected_group_sink_or_request_omission_reports_live_materialized_group(
    selected_group_sink_status: Option<&SinkStatusSnapshot>,
    request_scoped_schedule_omitted_ready_groups: Option<&BTreeSet<String>>,
    group_id: &str,
    query_path: &[u8],
) -> bool {
    selected_group_sink_status_reports_live_materialized_group(selected_group_sink_status, group_id)
...
fn selected_group_empty_ready_tree_requires_proxy_fallback(
    ...
    matches!(params.op, QueryOp::Tree)
        && params
            ...
            .is_some_and(|options| options.read_class == ReadClass::TrustedMaterialized)
        && path == b"/"
        && selected_group_sink_or_request_omission_reports_live_materialized_group(
            selected_group_sink_status,
            request_scoped_schedule_omitted_ready_groups,
            group_id,
            path,
...
enum TreePitStageExecutionAction {
    PushPayload {
        ...
        request_scoped_omitted_ready_root_requires_entries: bool,
    },
    PushEmptySnapshot,
    PushErrorSnapshot(CnxError),
    ReturnError(CnxError),
...
impl TreePitSessionPlan {
    ...
    fn request_scoped_sink_status_plan(self) -> TreePitRequestScopedSinkStatusPlan {
        TreePitRequestScopedSinkStatusPlan {
            ...
            use_ready_cache_fallback: true,
        }
    }

    fn fail_closed_request_scoped_sink_status_plan(self) -> TreePitRequestScopedSinkStatusPlan {
        TreePitRequestScopedSinkStatusPlan {
            use_ready_cache_fallback: false,
            ..self.request_scoped_sink_status_plan()
        }
    }
    ...
    fn tree_entry_action_at(
        ...
        if let Some(node_id) = selected_runner {
            ForceFindTreeRouteAction::QueryViaSelectedRunner {
                node_id,
                route_plan: attempt_runtime.selected_runner_delivery_plan(),
            }
        } else {
...
impl ForceFindRouteAttemptRuntime {
    ...
    fn selected_runner_delivery_plan(self) -> ForceFindRoutePlan {
        self.route_plan.rebudget(
            self.remaining
                .saturating_sub(self.same_group_recovery_reserve()),
        )
    }

    fn same_group_recovery_reserve(self) -> Duration {
        self.remaining / 2
    }
...
impl TreePitRequestScopedSinkStatusPlan {
    ...
    fn use_ready_cache_fallback(self) -> bool {
        self.use_ready_cache_fallback
    }
}
...
async fn rescue_trusted_materialized_empty_tree_response(
    ...
    let response_requires_ready_root_entry_rescue =
        trusted_materialized_tree_response_requires_ready_root_entry_rescue(
            response,
            input.tree_params.tree_options.as_ref(),
            input.group_plan.stage_plan.prior_materialized_group_decoded,
            input.selected_group_sink_status,
            input.request_scoped_schedule_omitted_ready_groups,
            input.group_key,
            input.query_path,
        );
    let response_requires_proxy_rescue = trusted_materialized_tree_payload_is_empty(response)
        || response_requires_ready_root_entry_rescue;
    let should_attempt_proxy_rescue = matches!(
        input.empty_response_plan.lane,
        EmptyTreeRescueLane::OwnerRetryThenProxyFallback | EmptyTreeRescueLane::ProxyFallback
    ) || response_requires_ready_root_entry_rescue;
    if should_attempt_proxy_rescue
        && response_requires_proxy_rescue
        && let QueryBackend::Route {
            boundary,
            origin_id,
            ..
            ...
            .await
            {
                Ok(proxy_events) => match decode_materialized_selected_group_response(
                    ...
                    Ok(proxy_response) => {
                        ...
                        } else if response_requires_ready_root_entry_rescue {
                            return Err(
    ...
    if trusted_materialized_tree_response_requires_ready_root_entry_rescue(
        response,
        input.tree_params.tree_options.as_ref(),
        input.group_plan.stage_plan.prior_materialized_group_decoded,
        input.selected_group_sink_status,
...
struct TreePitStageExecutionInput<'a> {
    ...
    tree_params: InternalQueryRequest,
    events: &'a [Event],
    events_from_generic_proxy_fallback: bool,
    selected_group_sink_status: Option<&'a SinkStatusSnapshot>,
    group_plan: TreePitGroupPlan,
...
async fn resolve_tree_pit_stage_execution_action(
    ...
    match decode_materialized_selected_group_response(
        ...
        Ok(mut response) => {
            ...
            let settle_empty_after_prior_exact_file_decode =
                trusted_materialized_exact_file_lane_settles_empty_after_prior_decode(
                    &input.tree_params,
                    input.selected_group_sink_status,
                    input.group_key,
                    input.group_plan.prior_materialized_exact_file_decoded,
                );
            let settle_empty_after_prior_proxy_fallback = input.events_from_generic_proxy_fallback
                && input.group_plan.read_class == ReadClass::TrustedMaterialized
                && input.group_plan.stage_plan.prior_materialized_group_decoded
                && !input.group_plan.stage_plan.empty_root_requires_fail_closed;
            if input.group_plan.stage_plan.requires_rescue
                ...
                && !settle_empty_after_prior_exact_file_decode
                && !settle_empty_after_prior_proxy_fallback
                && let Err(err) = rescue_trusted_materialized_empty_tree_response(
                    TrustedMaterializedEmptyTreeResponseRescueInput {
                        state: input.state,
                        policy: input.policy,
                        tree_params: input.tree_params.clone(),
            ...
            let request_scoped_omitted_ready_root_requires_entries =
                trusted_materialized_tree_response_requires_ready_root_entry_rescue(
                    &response,
                    input.tree_params.tree_options.as_ref(),
                    input.group_plan.stage_plan.prior_materialized_group_decoded,
            ...
            TreePitStageExecutionAction::PushPayload {
                ...
                payload_limit_bytes,
                empty_response_plan,
                request_scoped_omitted_ready_root_requires_entries,
            }
        }
...
fn finalize_trusted_materialized_tree_group_response(
    ...
    if request_scoped_omitted_ready_root_requires_entries {
        return Err(trusted_materialized_unavailable_selected_group_tree_error(
            &group_key,
        ));
    }
...
async fn handle_retryable_tree_pit_stage_gap(
                ...
                .await
                {
                    Ok(proxy_events) => {
                        match decode_materialized_selected_group_response(
                            ...
                            Ok(proxy_response) => {
                                let proxy_response_requires_ready_root_entry_rescue =
                                    trusted_materialized_tree_response_requires_ready_root_entry_rescue(
                                        &proxy_response,
                                        input.tree_params.tree_options.as_ref(),
                                        group_plan.stage_plan.prior_materialized_group_decoded,
                                ...
                                if !empty_root_requires_fail_closed
                                    || (!trusted_materialized_tree_payload_is_empty(
                                        &proxy_response,
                                    ) && !proxy_response_requires_ready_root_entry_rescue)
                                {
                                    push_materialized_tree_group_payload(
                                        groups,
                                        group_rank_metrics,
                                ...
                                if proxy_response_requires_ready_root_entry_rescue {
                                    return Err(
                                        trusted_materialized_unavailable_selected_group_tree_error(
                                            &input.group_key,
                                        ),
    ...
    if trusted_materialized_tree_response_requires_ready_root_entry_rescue(
        &empty_materialized_tree_group_payload(input.query_path),
        input.tree_params.tree_options.as_ref(),
        group_plan.stage_plan.prior_materialized_group_decoded,
        input.selected_group_sink_status,
        input.request_scoped_schedule_omitted_ready_groups,
...
struct TreePitRankedGroupExecutionInput<'a> {
    ...
    read_class: ReadClass,
    total_ranked_groups: usize,
    defer_unselected_group_key_root_groups: bool,
}
...
#[test]
fn tree_pit_group_plan_preserves_proxy_fallback_min_budget_for_middle_groups() {
...
#[test]
fn tree_pit_stage_runtime_fail_closes_request_scoped_omitted_root_decode_gaps() {
    let stage_runtime = TreePitStageTiming {
    ...
    assert!(
        ...
        "request-scoped omitted-ready root gaps must fail closed instead of synthesizing an empty trusted-materialized root page",
    );
}

#[test]
fn request_scoped_omitted_ready_root_payload_without_entries_requires_entry_rescue() {
    let mut payload = TreeGroupPayload {
    ...
    let omitted_ready_groups = BTreeSet::from([String::from("nfs2")]);
    let first_page_options = TreeQueryOptions {
        read_class: ReadClass::TrustedMaterialized,
        entry_offset: 0,
        entry_limit: Some(1001),
        payload_limit_bytes: Some(MATERIALIZED_TREE_ROUTE_PAYLOAD_BYTES_MAX),
    };
    ...
    assert!(
        ...
        "request-scoped omitted-ready root pages must not settle without bounded entry evidence",
    );

    payload.entries.push(TreePageEntry {
        path: b"/child.txt".to_vec(),
        size: 7,
        modified_time_us: 2,
        is_dir: false,
        has_children: false,
    ...
    assert!(
        ...
        "entry evidence satisfies the request-scoped omitted-ready root rescue gate",
    );
}
...
fn materialized_status_load_plan_owns_canonical_route_timing() {
    ...
    assert_eq!(
        ...
        StatusRoutePlan::new(Duration::from_secs(30), STATUS_ROUTE_COLLECT_IDLE_GRACE),
        "owner-scoped sink-status fan-in must keep the correlation open long enough for same-route worker owner replies after facade replies",
    );
}
...
#[test]
fn trusted_materialized_cache_ready_is_not_discarded_before_tree_settle() {
    let source = include_str!("api.rs");
    let start = source
        .find("\nfn cached_trusted_materialized_fail_closed_status_snapshots(")
        .expect("cached trusted status helper exists");
    let end = source[start..]
        .find("\nfn cache_trusted_materialized_ready_status_snapshots(")
        .map(|offset| start + offset)
        .expect("ready cache store helper follows trusted cache helper");
    ...
    assert!(
        ...
        "trusted-ready status cache must be usable by /tree instead of forcing a fresh 5s readiness settle",
    );
    assert!(
        body.contains("Ok(Some((source_status, sink_status)))"),
        "trusted status cache helper should return the cached source/sink evidence once source candidates and sink-ready coverage are present",
    );
}

#[test]
fn trusted_materialized_status_load_short_circuits_when_route_fanin_is_ready() {
    let source = include_str!("api.rs");
    let start = source
        .find("\nasync fn load_materialized_status_snapshots_with_plan(")
        .expect("materialized status load helper exists");
    let end = source[start..]
        .find("\n#[cfg(test)]\nasync fn load_materialized_status_snapshots(")
        .map(|offset| start + offset)
        .expect("test-only load helper follows materialized status load");
    let body = &source[start..end];
    let ready_short_circuit = body
        .find("if status_load_plan.require_trusted_cache\n        && trusted_materialized_status_covers_readiness_groups(")
        .expect("trusted readiness load should short-circuit once route fan-in is already complete");
    let source_reaugment_after_sink_ready = body
        .find("&& sink_status_covers_ready_readiness_groups(&sink_status, &readiness_groups)")
        .expect("legacy source re-augment branch exists");

    assert!(
        ready_short_circuit < source_reaugment_after_sink_ready,
        "trusted /tree preflight should not spend another source-status route budget after the first fan-in already proves trusted readiness",
    );
}
...
fn force_find_route_runtime_owns_tree_entry_action() {
    ...
    assert_eq!(
        ...
        ForceFindTreeRouteAction::QueryViaSelectedRunner {
            ...
            route_plan: ForceFindRoutePlan {
                route_timeout: Duration::from_millis(1500),
                collect_idle_grace: Duration::from_millis(1500),
                retry_backoff: FORCE_FIND_ROUTE_RETRY_BACKOFF,
                min_inflight_hold: Duration::from_millis(1500),
            },
        },
        "force-find runtime should own the selected-runner delivery plan and leave same-group recovery budget instead of letting one selected route consume the whole group budget",
    );
    assert_eq!(
...
#[test]
fn multi_group_force_find_dispatches_ranked_groups_as_independent_branches() {
    let source = include_str!("api.rs");
    let start = source
        .find("\nimpl ForceFindPitSessionMachine")
        .expect("ForceFindPitSessionMachine impl exists");
    let end = source[start..]
    ...
    let body = &source[start..end];
    assert!(
        body.contains("join_all(self.rankings.iter().cloned().map(|rank|"),
        "multi-group force-find must start one independent group branch per ranked group instead of serializing groups behind the slowest selected runner"
    );
    assert!(
        !body.contains("for rank in self.rankings.iter().cloned()"),
        "multi-group force-find must not loop through groups sequentially; the public freshness envelope is a per-group fan-out contract"
    );
}
...
#[test]
fn preserve_loaded_ready_groups_for_request_scoped_service_gate_keeps_loaded_trusted_groups() {
    let loaded_sink_status = SinkStatusSnapshot {
        scheduled_groups_by_node: BTreeMap::from([
    ...
    let request_scoped_regression = SinkStatusSnapshot {
        scheduled_groups_by_node: BTreeMap::from([(
            "node-a".to_string(),
            vec!["nfs1".to_string(), "nfs3".to_string()],
        )]),
        groups: vec![
            crate::sink::SinkGroupStatusSnapshot {
                group_id: "nfs1".to_string(),
    ...
    assert!(
        sink_status_covers_ready_readiness_groups(&preserved, &readiness_groups),
        "request-scoped sink-status is a route-local overlay and must not close the whole-root trusted service gate after loaded status already proved ready groups: {preserved:?}"
    );
}
...
#[derive(Debug)]
struct SelectedGroupMaterializedEvents {
    events: Vec<Event>,
    from_generic_proxy_fallback: bool,
}

impl SelectedGroupMaterializedEvents {
    fn direct(events: Vec<Event>) -> Self {
        Self {
            events,
            from_generic_proxy_fallback: false,
        }
    }

    fn generic_proxy_fallback(events: Vec<Event>) -> Self {
        Self {
            events,
            from_generic_proxy_fallback: true,
        }
    }
}
...
async fn query_materialized_events_via_candidate_owner_nodes(
    ...
    let candidate_nodes = current_owner_candidate_nodes_for_groups_and_source_status(
        source,
        source_status,
        sink_status,
        &groups,
...
async fn query_materialized_events_with_selected_group_owner_snapshot_and_request_scoped_omissions(
    ...
    Ok(query_materialized_events_with_selected_group_owner_snapshot_and_request_scoped_omissions_with_route_info(
        state,
        policy,
        params,
        timeout,
...
async fn query_materialized_events_with_selected_group_owner_snapshot_and_request_scoped_omissions_with_route_info(
    state: &ApiState,
    policy: &ProjectionPolicy,
    params: InternalQueryRequest,
    timeout: Duration,
    ...
    request_scoped_schedule_omitted_ready_groups: Option<&BTreeSet<String>>,
    group_plan: TreePitGroupPlan,
) -> Result<SelectedGroupMaterializedEvents, CnxError> {
    match &state.backend {
        QueryBackend::Route {
            sink,
            boundary,
                ...
                && selected_group_owner_collection_gap
            {
                ...
                if let Some(events) = candidate_owner_outcome.events {
                    if !selected_group_events_require_proxy_fallback(
                        ...
                        return Ok(SelectedGroupMaterializedEvents::direct(events));
                    }
                }
                let remaining = deadline
                    .checked_duration_since(tokio::time::Instant::now())
                ...
                if let Some(primary_node_id) =
                    sink_primary_owner_node_for_group(selected_group_sink_status.as_ref(), group_id)
                        .filter(|primary_node_id| {
                            group_plan.read_class != ReadClass::TrustedMaterialized
                        ...
                        })
                {
                    ...
                    {
                        return Ok(SelectedGroupMaterializedEvents::direct(primary_events));
                    }
                }
                let remaining = deadline
                    .checked_duration_since(tokio::time::Instant::now())
                ...
                return Ok(SelectedGroupMaterializedEvents::generic_proxy_fallback(
                ...
                .map_err(QueryWorkerObservationFailure::into_error)?
            {
                ...
                {
                    Ok(events) => {
                        return Ok(SelectedGroupMaterializedEvents::direct(
                            finalize_selected_group_owner_success(
                                policy,
                                &params,
                                deadline,
                                sink,
            ...
            if params.scope.selected_group.is_some() {
                if let Some(group_id) = params.scope.selected_group.as_deref()
                    ...
                    && !request_scoped_omitted_ready_root_group_requires_materialized_query(
                        &params,
                        selected_group_sink_status.as_ref(),
                        request_scoped_schedule_omitted_ready_groups,
                        group_id,
                ...
                {
                    return Ok(SelectedGroupMaterializedEvents::direct(Vec::new()));
                }
                let remaining = deadline
                    .checked_duration_since(tokio::time::Instant::now())
                    .unwrap_or_default();
            ...
            Ok(SelectedGroupMaterializedEvents::direct(
                query_materialized_events(state.backend.clone(), params, timeout).await?,
            ))
        }
        QueryBackend::Local { .. } => Ok(SelectedGroupMaterializedEvents::direct(
            query_materialized_events(state.backend.clone(), params, timeout).await?,
        )),
    }
}
...
async fn route_force_find_events_via_node(
    ...
    let adapter = exchange_host_adapter(
        boundary,
        caller_node_id,
        source_find_route_bindings_for(&runner_node_id.0),
    );
    let payload = rmp_serde::to_vec_named(&params)
        ...
        match phase {
            ...
            ForceFindRouteMachinePhase::AttemptSelectedRunner {
                ...
                let attempt = if matches!(
                    ...
                    let caller_node_id = match &state.backend {
                        QueryBackend::Route { origin_id, .. } => origin_id.clone(),
                        QueryBackend::Local { .. } => node_id.clone(),
                    };
                    run_timed_query(
...
fn trusted_materialized_tree_response_requires_ready_root_entry_rescue(
    payload: &TreeGroupPayload,
    _tree_options: Option<&TreeQueryOptions>,
    _prior_materialized_group_decoded: bool,
    _selected_group_sink_status: Option<&SinkStatusSnapshot>,
    request_scoped_schedule_omitted_ready_groups: Option<&BTreeSet<String>>,
    group_key: &str,
    query_path: &[u8],
) -> bool {
    if !(query_path.is_empty() || query_path == b"/") || !payload.entries.is_empty() {
        return false;
    }
    if request_scoped_schedule_omitted_ready_groups.is_some_and(|groups| groups.contains(group_key))
    {
        return true;
    }
    false
...
fn deferred_materialized_tree_group_snapshot(
    read_class: ReadClass,
    group_key: String,
) -> GroupPitSnapshot {
    GroupPitSnapshot {
        ...
        meta: PitMetadata {
            read_class,
            metadata_available: false,
            withheld_reason: Some("group-deferred-until-page"),
        },
        root: None,
        entries: Vec::new(),
...
fn tree_pit_snapshot_is_deferred(snapshot: &GroupPitSnapshot) -> bool {
...
#[test]
fn trusted_materialized_root_readiness_gap_rejects_partial_ready_coverage() {
    ...
    let message = trusted_materialized_root_readiness_gap_message(
        b"/",
        None,
        None,
        &source_status,
    ...
    .expect("partial root readiness must fail closed");

    assert!(message.contains("sink schedule missing [nfs2]"));
    assert!(message.contains("sink readiness missing [nfs2]"));
}

#[test]
fn trusted_materialized_root_readiness_gap_uses_normalized_sink_readiness() {
    let source_status = SourceStatusSnapshot {
    ...
    assert!(
        ...
        .is_none(),
        "live materialized sink evidence must use normalized readiness instead of stale raw enum state"
    );
}
...
fn trusted_materialized_root_readiness_gap_uses_authoritative_roots_when_source_status_is_partial()
{
    ...
    let expected_roots = BTreeSet::from(["nfs1".to_string(), "nfs2".to_string()]);
    let message = trusted_materialized_root_readiness_gap_message(
        b"/",
        None,
        Some(&expected_roots),
    ...
    .expect("partial source-status must not shrink unscoped root trusted-read coverage");

    assert!(message.contains("source coverage missing [nfs2]"));
    assert!(message.contains("sink schedule missing [nfs2]"));
    assert!(message.contains("sink readiness missing [nfs2]"));
}
...
fn merge_source_status_snapshots_preserves_completed_audit_evidence_from_older_snapshot() {
    let audited_root = crate::source::SourceConcreteRootHealthSnapshot {
        ...
        last_audit_completed_at_us: Some(20),
        last_audit_duration_ms: Some(10),
        emitted_batch_count: 4,
        emitted_event_count: 12,
        emitted_control_event_count: 1,
        emitted_data_event_count: 11,
        emitted_path_capture_target: None,
        emitted_path_event_count: 8,
        last_emitted_at_us: Some(30),
        last_emitted_origins: vec!["node-a::nfs1=4".to_string()],
        forwarded_batch_count: 4,
        forwarded_event_count: 12,
        forwarded_path_event_count: 8,
        last_forwarded_at_us: Some(31),
        last_forwarded_origins: vec!["node-a::nfs1=4".to_string()],
        current_revision: Some(1),
        current_stream_generation: Some(1),
        candidate_revision: None,
        candidate_stream_generation: None,
    ...
    let fresher_root = crate::source::SourceConcreteRootHealthSnapshot {
        root_key: audited_root.root_key.clone(),
        logical_root_id: audited_root.logical_root_id.clone(),
        object_ref: audited_root.object_ref.clone(),
        status: "serving".to_string(),
        coverage_mode: audited_root.coverage_mode.clone(),
        watch_enabled: audited_root.watch_enabled,
        scan_enabled: audited_root.scan_enabled,
        is_group_primary: audited_root.is_group_primary,
        active: audited_root.active,
        watch_lru_capacity: audited_root.watch_lru_capacity,
        audit_interval_ms: audited_root.audit_interval_ms,
        ...
        emitted_batch_count: audited_root.emitted_batch_count,
        emitted_event_count: audited_root.emitted_event_count,
        emitted_control_event_count: audited_root.emitted_control_event_count,
        emitted_data_event_count: audited_root.emitted_data_event_count,
        emitted_path_capture_target: None,
        emitted_path_event_count: audited_root.emitted_path_event_count,
        last_emitted_at_us: audited_root.last_emitted_at_us,
        last_emitted_origins: audited_root.last_emitted_origins.clone(),
        forwarded_batch_count: audited_root.forwarded_batch_count,
        forwarded_event_count: audited_root.forwarded_event_count,
        forwarded_path_event_count: audited_root.forwarded_path_event_count,
        last_forwarded_at_us: audited_root.last_forwarded_at_us,
        last_forwarded_origins: audited_root.last_forwarded_origins.clone(),
    ...
    let merged = merge_source_status_snapshots(vec![
        SourceStatusSnapshot {
            concrete_roots: vec![audited_root],
            ..SourceStatusSnapshot::default()
        },
        SourceStatusSnapshot {
            concrete_roots: vec![fresher_root],
            ..SourceStatusSnapshot::default()
        },
    ]);
    ...
    let merged_root = merged
        ...
        .expect("merged root should be preserved");

    assert_eq!(merged_root.current_revision, Some(2));
...
fn render_pit_response(
    ...
    let selected = session
        ...
        .collect::<Vec<_>>();
    if let Some(message) =
        trusted_materialized_tree_group_page_not_ready_message(session, &selected)
    {
...
fn trusted_materialized_tree_group_not_ready_message(
    read_class: ReadClass,
...
fn trusted_materialized_tree_group_page_not_ready_message(
    session: &PitSession,
    selected: &[&GroupPitSnapshot],
) -> Option<String> {
    ...
    for group in selected {
        if let Some(message) =
            trusted_materialized_tree_group_not_ready_message(session.read_class, group)
        {
            return Some(message);
        }
    }
...
fn trusted_materialized_tree_session_page_not_ready_message(
    session: &PitSession,
    group_offset: usize,
    group_page_size: usize,
) -> Option<String> {
...
fn tree_pit_snapshot_needs_entry_window(
    ...
    if tree_pit_snapshot_is_deferred(snapshot) {
        return true;
    }
    snapshot.meta.metadata_available
...
fn merge_tree_pit_entry_window(
    ...
    if tree_pit_snapshot_is_deferred(snapshot) {
        let read_class = snapshot.meta.read_class;
        snapshot.status = "ok";
        snapshot.reliable = payload.reliability.reliable;
        snapshot.unreliable_reason = payload.reliability.unreliable_reason;
        snapshot.stability = payload.stability;
        snapshot.meta = PitMetadata {
            read_class,
            metadata_available: true,
            withheld_reason: None,
        };
        snapshot.root = Some(payload.root);
        snapshot.entries = payload.entries;
        snapshot.entry_window_complete = entry_window_complete;
...
async fn extend_tree_pit_session_entry_windows(
    ...
    if !changed {
        if let Some(message) = trusted_materialized_tree_session_page_not_ready_message(
            &session,
            group_offset,
            group_page_size,
        ) {
...
async fn build_tree_pit_session_with_request_scoped_schedule_omitted_ready_groups(
    ...
    prepare_tree_pit_session_machine(
        ...
        request_sink_status,
        request_scoped_schedule_omitted_ready_groups,
        defer_unselected_group_key_root_groups,
    )
    .await?
    .run()
    .await
...
impl TreePitGroupMachine<'_, '_> {
    async fn run(self) -> Result<(), CnxError> {
        ...
        let routed_events = match tokio::time::timeout(
            stage_timeout,
            query_materialized_events_with_selected_group_owner_snapshot_and_request_scoped_omissions_with_route_info(
                input.state,
                input.policy,
                tree_params.clone(),
                stage_timeout,
        ...
        apply_tree_pit_stage_execution_action(
            resolve_tree_pit_stage_execution_action(TreePitStageExecutionInput {
                ...
                policy: input.policy,
                tree_params: tree_params.clone(),
                events: &routed_events.events,
                events_from_generic_proxy_fallback: routed_events.from_generic_proxy_fallback,
                selected_group_sink_status: selected_group_sink_status.as_ref(),
                group_plan,
                stage_timing,
                selected_group_owner_known,
...
impl TreePitSessionMachine<'_> {
    ...
    async fn run_ranked_groups(&self) -> Result<Vec<GroupPitSnapshot>, CnxError> {
        ...
        let materialized_group_limit = tree_pit_initial_materialized_group_limit(
            self.input.params,
            self.rankings.len(),
            self.input.defer_unselected_group_key_root_groups,
        )?;
        for (rank_index, rank) in self.rankings.iter().cloned().enumerate() {
            if rank_index >= materialized_group_limit {
                state.groups.push(deferred_materialized_tree_group_snapshot(
                    self.input.read_class,
                    rank.group_key,
                ));
                continue;
            }
...
fn tree_pit_initial_materialized_group_limit(
    params: &NormalizedApiParams,
    total_ranked_groups: usize,
    defer_unselected_group_key_root_groups: bool,
) -> Result<usize, CnxError> {
    if defer_unselected_group_key_root_groups
        && params.group.is_none()
        && params.group_order == GroupOrder::GroupKey
        && trusted_materialized_empty_group_root_requires_fail_closed(&params.path)
    {
        return Ok(normalize_group_page_size(params)?.min(total_ranked_groups));
    }
    Ok(total_ranked_groups)
}
...
impl ForceFindPitSessionMachine<'_> {
    ...
    async fn run_ranked_groups(&self) -> Result<Vec<GroupPitSnapshot>, CnxError> {
        join_all(self.rankings.iter().cloned().map(|rank| {
...
async fn query_tree_page_response(
    ...
    let session = extend_tree_pit_session_entry_windows(
        ...
        &entry_offsets,
        entry_page_size,
        remaining_timeout_from(request_started_at, timeout)?,
        request_source_status.as_ref(),
        request_sink_status.as_ref(),
        request_scoped_schedule_omitted_ready_groups.as_ref(),
    )
...
async fn get_tree(
    ...
    let (request_sink_status, request_scoped_schedule_omitted_ready_groups) = if read_class
        == ReadClass::TrustedMaterialized
    {
        let root_trusted_tree =
            trusted_materialized_empty_group_root_requires_fail_closed(&params.path);
        let request_scoped_plan = if root_trusted_tree {
            TreePitSessionPlan::new(policy.query_timeout(), 1)
                .fail_closed_request_scoped_sink_status_plan()
        } else {
            TreePitSessionPlan::new(policy.query_timeout(), 1).request_scoped_sink_status_plan()
        };
        let request_scoped = match load_request_scoped_materialized_sink_status_snapshot(
            &state,
            request_scoped_plan,
        )
        .await
        {
            Some(request_scoped) => request_scoped,
            None if root_trusted_tree => {
                return error_response_with_context(
                    CnxError::NotReady(
                        "trusted-materialized root tree remains unavailable until request-scoped sink status is available"
                            .into(),
                    ),
                    Some(&path_for_error),
                );
            }
            None => sink_status.clone(),
        };
