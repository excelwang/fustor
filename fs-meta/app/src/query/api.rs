...
async fn load_materialized_status_snapshots(
        ...
        && let Some(local_sink) = &state.readiness_sink
    {
        if let Ok(local_sink_status) = local_sink.cached_status_snapshot() {
            let local_sink_status = filter_sink_status_snapshot(local_sink_status, &readiness_groups);
            if sink_status_snapshot_has_better_groups(&sink_status, &local_sink_status) {
                sink_status = merge_sink_status_snapshots(vec![sink_status, local_sink_status]);
                let mut cache = state.materialized_sink_status_cache.lock().map_err(|_| {
                    CnxError::Internal("materialized sink status cache lock poisoned".into())
                })?;
                *cache = Some(CachedSinkStatusSnapshot {
...
fn fresh_sink_group_explicitly_empty(
...
fn source_status_group_still_active(
    source_status: &SourceStatusSnapshot,
    group_id: &str,
) -> bool {
    source_status
        ...
        || source_status.concrete_roots.iter().any(|root| {
            root.logical_root_id == group_id && root.active && root.scan_enabled
        })
}
...
fn selected_group_materialized_tree_payload_is_empty(
    ...
    fn payload_precedence(event: &Event, payload: &TreeGroupPayload) -> (u64, u8, usize, bool, u64)
    {
...
fn selected_group_owner_attempt_timeout(deadline: tokio::time::Instant) -> Duration {
    ...
    let owner_min_budget = std::cmp::min(remaining, TRUSTED_READY_SELECTED_GROUP_RETRY_BUDGET);
    ...
    let capped_proxy_reserve = std::cmp::min(proxy_reserve, remaining.saturating_sub(owner_min_budget));
                ...
                .await?
            {
                if node_id == *origin_id {
                    let owner_attempt_timeout = if reserve_proxy_budget {
                        selected_group_owner_attempt_timeout(deadline)
                    } else {
                        deadline
                            .checked_duration_since(tokio::time::Instant::now())
                            .unwrap_or_default()
                    ...
                    if !requires_proxy_fallback {
                        if requires_primary_owner_reroute
                            && let Some(primary_node_id) =
                                sink_primary_owner_node_for_group(
                                    selected_group_sink_status.as_ref(),
                                    group_id,
                                )
                    ...
                    if !reserve_proxy_budget && !allow_empty_owner_retry {
                        return Err(trusted_materialized_empty_selected_group_tree_error(group_id));
                    }
                    let remaining = deadline
                        .checked_duration_since(tokio::time::Instant::now())
                        .unwrap_or_default();
...
async fn query_materialized_events_via_selected_group_owner_direct(
    ...
    match &state.backend {
        ...
        QueryBackend::Local { .. } => query_materialized_events(state.backend.clone(), params, timeout).await,
    }
}
...
async fn materialized_target_groups(
    ...
    let scheduled_groups = if let Some(snapshot) = request_sink_status {
        Some(materialized_scheduled_group_ids(&filter_sink_status_snapshot(
            snapshot.clone(),
            &allowed_groups,
        )))
    } else {
        ...
        .map(|snapshot| {
            ...
            let merged =
                merge_with_cached_sink_status_snapshot(cache.as_ref(), &allowed_groups, snapshot);
            *cache = Some(CachedSinkStatusSnapshot {
                        ...
                        .await?
                {
                    ...
                    match result {
                                ...
                                && tokio::time::Instant::now() < deadline =>
                        {
                            if debug_force_find_route_capture_enabled() {
                                eprintln!(
                                    "fs_meta_query_api: force-find tree route_fallback retry group={} err={}",
                                    group_id, err
                                );
                            }
                            tokio::time::sleep(FORCE_FIND_ROUTE_RETRY_BACKOFF).await;
...
async fn build_tree_pit_session(
    ...
    let target_groups =
        materialized_target_groups(state, params.group.as_deref(), request_sink_status, timeout)
            .await?;
    ...
    let session_deadline = tokio::time::Instant::now() + timeout;
    ...
    for (rank_index, rank) in rankings.into_iter().enumerate() {
        ...
        let allow_empty_owner_retry = trusted_materialized_ready_group
                ...
                if trusted_materialized_ready_group {
                    ...
                    } = &state.backend
                    {
                        let remaining = session_deadline
                            ...
                            .unwrap_or_default();
                        if !remaining.is_zero() {
                            let proxy_retry_timeout = std::cmp::min(
                                remaining,
                            ...
                            if !proxy_retry_timeout.is_zero()
                                && let Ok(proxy_events) = run_timed_query(
                                    query_materialized_events_via_generic_proxy(
                                        boundary.clone(),
                                        origin_id.clone(),
                                        tree_params.clone(),
                                    ...
                                    )
                            {
                                    ...
                                    )
                                {
                                    ...
                                    groups.push(GroupPitSnapshot {
                                        group: group_key,
                                        status: "ok",
                                        reliable: proxy_response.reliability.reliable,
                                        ...
                                        root: metadata_available.then_some(proxy_response.root),
        ...
        match decode_materialized_selected_group_response(&events, policy, &group_key, &params.path)
        {
            Ok(mut response) => {
                    ...
                    if !remaining.is_zero() {
                            ...
                            } = &state.backend
                        {
                            ...
                            if !remaining.is_zero() {
                                ...
                                if let Ok(proxy_events) = run_timed_query(
                                    query_materialized_events_via_generic_proxy(
                                        boundary.clone(),
                                        origin_id.clone(),
                                        tree_params.clone(),
                ...
                if trusted_materialized_ready_group
                    ...
                    && trusted_materialized_tree_payload_is_empty(&response)
                {
                    return Err(trusted_materialized_empty_selected_group_tree_error(&group_key));
                }
...
mod tests {
    ...
    impl SourceStatusOkSinkStatusExplicitEmptyThenReadyBoundary {
        fn new(source_status_payload: Vec<u8>, sink_status_payloads: Vec<Vec<u8>>) -> Self {
            ...
            Self {
                ...
                sink_reply_channel: format!("{}:reply", sink_route.0),
                source_status_payload,
                sink_status_payloads: std::sync::Mutex::new(
                    sink_status_payloads.into_iter().collect(),
                ),
                send_batches_by_channel: std::sync::Mutex::new(HashMap::new()),
                recv_batches_by_channel: std::sync::Mutex::new(HashMap::new()),
                correlations_by_channel: std::sync::Mutex::new(HashMap::new()),
    ...
    impl ChannelIoSubset for MaterializedRouteMissingRouteStateThenProxyBoundary {
        ...
        async fn channel_recv(
            ...
            {
                let request_channel =
                    self.sink_status_reply_channel.trim_end_matches(":reply");
                let correlation = self
                    .correlations_by_channel
                    .lock()
                    .expect("materialized missing-route-state boundary correlations lock")
    ...
    impl ChannelIoSubset for MaterializedSelectedGroupTreeStallBoundary {
        ...
        async fn channel_recv(
            ...
            let source_status_reply_channel =
                format!("{}:reply", self.source_status_request_channel);
                        ...
                        "owner" => {
                            ...
                            match params.op {
                                ...
                                QueryOp::Tree => {
                                    ...
                                    {
                                        let payload = real_materialized_tree_payload_for_test(
                                            &params.scope.path,
                                        );
                                        return Ok(vec![mk_event_with_correlation(
                                            params
    ...
    fn real_materialized_exact_file_payload_for_test(path: &[u8], size: u64, modified_time_us: u64) -> Vec<u8> {
        rmp_serde::to_vec_named(&MaterializedQueryPayload::Tree(TreeGroupPayload {
            reliability: GroupReliability {
                reliable: true,
                unreliable_reason: None,
    ...
    async fn load_materialized_status_snapshots_preserves_cached_ready_groups_when_routed_sink_status_reports_explicit_empty_root_transition_snapshot()
    {
        ...
        let source = source_facade_with_roots(
            vec![RootSpec::new("nfs2", &node_a_root), RootSpec::new("nfs4", &node_c_root)],
            &grants,
        );
        ...
        let cached_snapshot = SinkStatusSnapshot {
            ...
            groups: vec![sink_group_status("nfs2", true), sink_group_status("nfs4", true)],
            ..SinkStatusSnapshot::default()
        };
        let state = ApiState {
            backend: QueryBackend::Route {
            ...
            readiness_source: Some(source),
            readiness_sink: Some(sink),
            materialized_sink_status_cache: Arc::new(Mutex::new(Some(
                CachedSinkStatusSnapshot {
    ...
    async fn load_materialized_status_snapshots_retries_routed_sink_status_when_first_reply_reports_all_active_groups_explicit_empty()
    {
        ...
        let ready_sink_status_payload = rmp_serde::to_vec_named(&SinkStatusSnapshot {
            ...
            groups: vec![sink_group_status("nfs1", true), sink_group_status("nfs2", true)],
            ..SinkStatusSnapshot::default()
        })
        .expect("encode ready sink-status payload");
        let decoded_source_status = rmp_serde::from_slice::<SourceObservabilitySnapshot>(
            &source_status_payload,
        )
        .expect("decode source-status payload for helper assertion")
        .status;
        let decoded_explicit_empty_sink_status = rmp_serde::from_slice::<SinkStatusSnapshot>(
        ...
        let boundary = Arc::new(SourceStatusOkSinkStatusExplicitEmptyThenReadyBoundary::new(
            source_status_payload,
            vec![explicit_empty_sink_status_payload, ready_sink_status_payload],
        ));
        let sink_status_route = default_route_bindings()
            .resolve(ROUTE_TOKEN_FS_META_INTERNAL, METHOD_SINK_STATUS)
            .expect("resolve sink-status route");
    ...
    async fn selected_group_force_find_route_reroutes_when_chosen_runner_reports_selected_group_missing()
     {
        ...
        fs::write(root_a.join("force-find-stress").join("seed.txt"), b"a")
            .expect("seed node-a file");
        fs::write(root_b.join("force-find-stress").join("seed.txt"), b"b")
            .expect("seed node-b file");
        let grants = vec![
            GrantedMountRoot {
                object_ref: "node-a::nfs1".to_string(),
                host_ref: "node-a".to_string(),
    ...
    async fn selected_group_materialized_route_reroutes_to_sink_primary_object_ref_when_scheduled_owner_returns_empty_tree_for_ready_group()
     {
        ...
        primary_owner_endpoint.shutdown(Duration::from_secs(2)).await;
        proxy_endpoint.shutdown(Duration::from_secs(2)).await;
    }
    ...
    async fn get_stats_trusted_materialized_keeps_request_sink_snapshot_when_selected_group_stats_route_would_otherwise_drift()
    {
        ...
        let request_sink_status_payload = rmp_serde::to_vec_named(&request_sink_status)
        .expect("encode request sink-status payload");
            ...
            move |requests| {
                ...
                async move {
                    requests
                        .into_iter()
                        .map(|req| {
                            let call_idx = sink_status_calls
                                .fetch_add(1, std::sync::atomic::Ordering::SeqCst);
        ...
        source_status_endpoint.shutdown(Duration::from_secs(2)).await;
        primary_owner_endpoint.shutdown(Duration::from_secs(2)).await;
        stale_owner_endpoint.shutdown(Duration::from_secs(2)).await;
    }
                ...
                async move {
                    ...
                    for req in requests {
                        ...
                        let group_id = params
                            ...
                            .expect("selected group for owner request");
                        let payload =
                            if owner_call_count.fetch_add(1, std::sync::atomic::Ordering::SeqCst)
                                == 0
                            {
                                empty_materialized_tree_payload_for_test(&params.scope.path)
                            } else {
    ...
    async fn build_tree_pit_session_trusted_materialized_uses_outer_retry_budget_when_later_ranked_group_first_reply_is_empty()
    {
        ...
        assert!(
            ...
            session
                .groups
                .iter()
                .map(|group| (group.group.clone(), group.root.as_ref().map(|root| root.exists)))
                .collect::<Vec<_>>()
        );
        assert_eq!(
            owner_call_count.load(std::sync::atomic::Ordering::SeqCst),
        ...
        assert!(
            ...
            session
                ...
                .map(|group| {
                    (
                        group.group.clone(),
                        group.root.as_ref().map(|root| (root.exists, root.path.clone())),
                    )
                })
                .collect::<Vec<_>>()
        );
            ...
            move |requests| {
                ...
                async move {
                    ...
                    for req in requests {
                        let params = rmp_serde::from_slice::<InternalQueryRequest>(req.payload_bytes())
                            .expect("decode proxy query request");
                        let group_id = params.scope.selected_group.clone().expect("proxy selected group");
                        if group_id == "nfs2" {
    ...
    async fn build_tree_pit_session_completes_when_one_ranked_group_tree_stalls_after_another_group_decodes()
     {
        ...
        let sink_status_payload = rmp_serde::to_vec_named(&request_sink_status)
        .expect("encode sink-status payload");
        let boundary = Arc::new(
    ...
    async fn build_tree_pit_session_completes_when_later_ranked_group_reply_is_delayed_but_within_shared_budget()
    {
        ...
        .await
        .expect("later-ranked delayed materialized reply should still settle within the shared PIT budget")
        .expect("build tree pit session");
    ...
    async fn build_tree_pit_session_trusted_materialized_bounds_exact_file_third_group_stage_after_first_group_and_empty_second_group()
    {
        ...
        let mut owner_b_endpoint = ManagedEndpointTask::spawn(
            ...
            move |_requests| async move {
                std::future::pending::<Vec<Event>>().await
            },
        );
        let state =
            test_api_state_for_route_source(source, sink, boundary, NodeId("node-d".to_string()));
    ...
    async fn build_tree_pit_session_trusted_materialized_falls_back_to_proxy_when_later_ranked_exact_file_owner_times_out_for_missing_path()
    {
        ...
        assert_eq!(
            group_roots,
            vec![
                ("nfs1".to_string(), Some((true, b"/nested/child/deep.txt".to_vec()))),
                ("nfs2".to_string(), Some((false, b"/nested/child/deep.txt".to_vec()))),
                ("nfs3".to_string(), Some((false, b"/nested/child/deep.txt".to_vec()))),
            ],
            ...
            .map(|group| {
                (
                    ...
                    group.root.as_ref().map(|root| {
                        (root.exists, root.path.clone(), root.is_dir, root.size, group.entries.len())
                    }),
                )
            })
            .collect::<Vec<_>>();
                ...
                requests
                    ...
                    .map(|req| {
                        ...
                        let payload = if group_id == "nfs1" {
                            ...
                            real_materialized_tree_payload_for_test_with_root_path(
                                b"/",
                                b"/data/a.txt",
                            )
                        };
    ...
    async fn public_trusted_tree_preserves_request_ready_sink_truth_when_later_pit_sink_status_regresses()
    {
        ...
        let payload: serde_json::Value =
            serde_json::from_slice(&body).expect("tree response json");
    ...
    async fn concurrent_public_tree_requests_both_settle_under_multi_group_selected_route_stall() {
        ...
        assert!(
            second_response.status().is_success() || second_response.status().is_server_error()
        );
    }
}
