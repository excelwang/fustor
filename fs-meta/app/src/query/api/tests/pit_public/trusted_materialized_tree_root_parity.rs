...
async fn public_trusted_tree_force_find_stress_path_ignores_unrelated_pending_group_readiness() {
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "trusted-materialized `/force-find-stress` should not fail closed on an unrelated pending-materialization nfs3 group when nfs1/nfs2 already materialize the requested path: {payload}"
    );
