...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn public_trusted_exact_file_path_b64_does_not_preserve_first_request_owner_when_later_sink_status_omits_group_from_schedule()
 {
    let tmp = tempfile::tempdir().expect("create tempdir");
    let root_a = tmp.path().join("node-a");
    let root_b = tmp.path().join("node-b");
    ...
    assert_eq!(
        second_group_roots,
        vec![
            ("nfs1".to_string(), false, path.to_string(), true, 0),
            ("nfs2".to_string(), false, path.to_string(), true, 0),
            ("nfs3".to_string(), false, path.to_string(), true, 0),
        ],
        "trusted exact-file path_b64 request must not preserve the first request's nfs1 materialized owner result once later sink-status omits nfs1 from the scheduled map: {second_payload}"
    );
